  ...
  return (
    <div ref={ref} style={{ position: "relative", flex: 1 }}>
      ...
      >
        ...
        <ChevronDown
          size={12} strokeWidth={1.8} color="rgba(0,0,0,0.25)"
      ...
      {open && (
        <div style={{
          ...
          backgroundColor: "#fff", borderRadius: 8, border: "1px solid rgba(0,0,0,0.06)",
          boxShadow: "0 4px 16px rgba(0,0,0,0.08), 0 1px 3px rgba(0,0,0,0.04)",
          padding: "4px", maxHeight: 180, overflowY: "auto",
        }}>
          {options.map((opt) => (
            <button
              key={opt.value}
              ...
              onMouseLeave={(e) => { e.currentTarget.style.backgroundColor = value === opt.value ? "rgba(220,38,38,0.04)" : "transparent"; }}
            >
              <span style={{ overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap", flex: 1 }}>{opt.label}</span>
              {value === opt.value && <Check size={12} strokeWidth={2.5} color="#DC2626" style={{ flexShrink: 0, marginLeft: 6 }} />}
            </button>
          ))}
        </div>
      )}
...
function ConditionalLogicEditor({
  ...
  return (
    <div style={{ paddingTop: 6 }}>
        ...
        return (
          <div key={rule.id} style={{ border: "1px solid rgba(0,0,0,0.05)", borderRadius: 10, padding: "12px 14px 14px", marginBottom: 8, backgroundColor: "#fff" }}>
            <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 12 }}>
              ...
              <button onClick={() => removeRule(rule.id)} style={{ background: "none", border: "none", cursor: "pointer", padding: 2, color: "rgba(0,0,0,0.2)", transition: "color 0.15s ease" }} onMouseEnter={(e) => (e.currentTarget.style.color = "#DC2626")} onMouseLeave={(e) => (e.currentTarget.style.color = "rgba(0,0,0,0.2)")}>
                <Trash2 size={12} strokeWidth={1.6} />
              </button>
            </div>
            <div style={{ display: "flex", alignItems: "center", marginBottom: 8 }}>
              <span style={clFieldLabel}>Wenn Frage</span>
              <CLDropdown value={rule.triggerQuestionId} options={triggerOptions} onChange={(v) => updateRule(rule.id, { triggerQuestionId: v, triggerValue: "", triggerValueMax: "" })} placeholder="Frage wählen..." />
            </div>
            <div style={{ display: "flex", alignItems: "center", marginBottom: 8 }}>
              <span style={clFieldLabel}>Operator</span>
              <CLDropdown value={rule.operator} options={operatorOptions} onChange={(v) => updateRule(rule.id, { operator: v, triggerValue: v === "between" ? "" : rule.triggerValue, triggerValueMax: "" })} />
            </div>
            <div style={{ display: "flex", alignItems: "center", marginBottom: 8 }}>
              ...
              {isBetween ? (
                ...
                <CLDropdown value={rule.triggerValue} options={answerOptions} onChange={(v) => updateRule(rule.id, { triggerValue: v })} placeholder="Antwort wählen..." />
              ) : (
                <div style={{ flex: 1 }}>
            ...
            <div style={{ display: "flex", alignItems: "center", marginBottom: 10 }}>
              ...
              <CLDropdown value={rule.action} options={actionOptions} onChange={(v) => updateRule(rule.id, { action: v as "hide" | "show" })} />
            </div>
            <div>
              ...
              {targetCandidates.length === 0 ? (
                ...
                <div style={{ display: "flex", flexDirection: "column", gap: 3 }}>
                  {targetCandidates.map((tq) => {
                    ...
                    return (
                      <button key={tq.id} onClick={() => toggleTarget(rule.id, tq.id)} style={{ display: "flex", alignItems: "center", gap: 8, fontSize: 11, fontWeight: 400, padding: "6px 8px", borderRadius: 6, border: "none", cursor: "pointer", transition: "background-color 0.12s ease", backgroundColor: isSelected ? "rgba(220,38,38,0.03)" : "transparent", color: "#374151", textAlign: "left", width: "100%" }} onMouseEnter={(e) => { if (!isSelected) e.currentTarget.style.backgroundColor = "rgba(0,0,0,0.02)"; }} onMouseLeave={(e) => { e.currentTarget.style.backgroundColor = isSelected ? "rgba(220,38,38,0.03)" : "transparent"; }}>
                        <div style={{ width: 16, height: 16, borderRadius: 4, border: isSelected ? "none" : "1.5px solid rgba(0,0,0,0.12)", backgroundColor: isSelected ? "#DC2626" : "transparent", display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0, transition: "all 0.12s ease" }}>
      ...
      <button onClick={() => onChange([...rules, newRule()])} style={{ display: "flex", alignItems: "center", justifyContent: "center", gap: 5, width: "100%", fontSize: 10, fontWeight: 600, color: "#DC2626", backgroundColor: "rgba(220,38,38,0.03)", border: "1px dashed rgba(220,38,38,0.15)", borderRadius: 8, cursor: "pointer", padding: "8px 0", transition: "all 0.15s ease" }} onMouseEnter={(e) => { e.currentTarget.style.backgroundColor = "rgba(220,38,38,0.06)"; e.currentTarget.style.borderColor = "rgba(220,38,38,0.25)"; }} onMouseLeave={(e) => { e.currentTarget.style.backgroundColor = "rgba(220,38,38,0.03)"; e.currentTarget.style.borderColor = "rgba(220,38,38,0.15)"; }}>
...
function Toggle({ value, onChange }: { value: boolean; onChange: (v: boolean) => void }) {
  return (
    <button onClick={() => onChange(!value)} style={{ width: 32, height: 18, borderRadius: 9, backgroundColor: value ? "#DC2626" : "rgba(0,0,0,0.12)", border: "none", cursor: "pointer", position: "relative", transition: "background-color 0.2s ease", flexShrink: 0 }}>
...
function LikertConfig({ config, onChange }: { config: Record<string, unknown>; onChange: (c: Record<string, unknown>) => void }) {
  return (
    <div style={{ marginTop: 10, display: "flex", gap: 12, flexWrap: "wrap" }}>
      <div style={{ flex: "1 1 60px" }}><span style={{ fontSize: 9, fontWeight: 600, textTransform: "uppercase", letterSpacing: "0.04em", color: "rgba(0,0,0,0.35)" }}>Min</span><input type="number" value={String(config.min ?? "")} placeholder="1" onChange={(e) => onChange({ ...config, min: e.target.value === "" ? "" : Number(e.target.value) })} style={{ display: "block", width: "100%", marginTop: 4, fontSize: 11, padding: "4px 0", border: "none", borderBottom: "1px solid rgba(0,0,0,0.08)", outline: "none", color: "#374151", backgroundColor: "transparent" }} /></div>
      <div style={{ flex: "1 1 60px" }}><span style={{ fontSize: 9, fontWeight: 600, textTransform: "uppercase", letterSpacing: "0.04em", color: "rgba(0,0,0,0.35)" }}>Max</span><input type="number" value={String(config.max ?? "")} placeholder="5" onChange={(e) => onChange({ ...config, max: e.target.value === "" ? "" : Number(e.target.value) })} style={{ display: "block", width: "100%", marginTop: 4, fontSize: 11, padding: "4px 0", border: "none", borderBottom: "1px solid rgba(0,0,0,0.08)", outline: "none", color: "#374151", backgroundColor: "transparent" }} /></div>
      <div style={{ flex: "1 1 120px" }}><span style={{ fontSize: 9, fontWeight: 600, textTransform: "uppercase", letterSpacing: "0.04em", color: "rgba(0,0,0,0.35)" }}>Min Label</span><input type="text" value={String(config.minLabel ?? "")} onChange={(e) => onChange({ ...config, minLabel: e.target.value })} placeholder="z.B. Sehr schlecht" style={{ display: "block", width: "100%", marginTop: 4, fontSize: 11, padding: "4px 0", border: "none", borderBottom: "1px solid rgba(0,0,0,0.08)", outline: "none", color: "#374151", backgroundColor: "transparent" }} /></div>
      <div style={{ flex: "1 1 120px" }}><span style={{ fontSize: 9, fontWeight: 600, textTransform: "uppercase", letterSpacing: "0.04em", color: "rgba(0,0,0,0.35)" }}>Max Label</span><input type="text" value={String(config.maxLabel ?? "")} onChange={(e) => onChange({ ...config, maxLabel: e.target.value })} placeholder="z.B. Sehr gut" style={{ display: "block", width: "100%", marginTop: 4, fontSize: 11, padding: "4px 0", border: "none", borderBottom: "1px solid rgba(0,0,0,0.08)", outline: "none", color: "#374151", backgroundColor: "transparent" }} /></div>
    </div>
  );
}

function NumericConfig({ config, onChange }: { config: Record<string, unknown>; onChange: (c: Record<string, unknown>) => void }) {
  return (
    <div style={{ marginTop: 10, display: "flex", gap: 12, alignItems: "flex-end", flexWrap: "wrap" }}>
      <div style={{ flex: "1 1 60px" }}><span style={{ fontSize: 9, fontWeight: 600, textTransform: "uppercase", letterSpacing: "0.04em", color: "rgba(0,0,0,0.35)" }}>Min</span><input type="number" value={String(config.min ?? "")} onChange={(e) => onChange({ ...config, min: e.target.value })} placeholder="—" style={{ display: "block", width: "100%", marginTop: 4, fontSize: 11, padding: "4px 0", border: "none", borderBottom: "1px solid rgba(0,0,0,0.08)", outline: "none", color: "#374151", backgroundColor: "transparent" }} /></div>
      <div style={{ flex: "1 1 60px" }}><span style={{ fontSize: 9, fontWeight: 600, textTransform: "uppercase", letterSpacing: "0.04em", color: "rgba(0,0,0,0.35)" }}>Max</span><input type="number" value={String(config.max ?? "")} onChange={(e) => onChange({ ...config, max: e.target.value })} placeholder="—" style={{ display: "block", width: "100%", marginTop: 4, fontSize: 11, padding: "4px 0", border: "none", borderBottom: "1px solid rgba(0,0,0,0.08)", outline: "none", color: "#374151", backgroundColor: "transparent" }} /></div>
      <div style={{ display: "flex", alignItems: "center", gap: 6, paddingBottom: 4 }}><Toggle value={!!config.decimals} onChange={(v) => onChange({ ...config, decimals: v })} /><span style={{ fontSize: 10, color: "#6b7280" }}>Dezimal</span></div>
    </div>
  );
}

function SliderConfig({ config, onChange }: { config: Record<string, unknown>; onChange: (c: Record<string, unknown>) => void }) {
  return (
    <div style={{ marginTop: 10, display: "flex", gap: 12, flexWrap: "wrap" }}>
      <div style={{ flex: "1 1 60px" }}><span style={{ fontSize: 9, fontWeight: 600, textTransform: "uppercase", letterSpacing: "0.04em", color: "rgba(0,0,0,0.35)" }}>Min</span><input type="number" value={String(config.min ?? "")} placeholder="0" onChange={(e) => onChange({ ...config, min: e.target.value === "" ? "" : Number(e.target.value) })} style={{ display: "block", width: "100%", marginTop: 4, fontSize: 11, padding: "4px 0", border: "none", borderBottom: "1px solid rgba(0,0,0,0.08)", outline: "none", color: "#374151", backgroundColor: "transparent" }} /></div>
      <div style={{ flex: "1 1 60px" }}><span style={{ fontSize: 9, fontWeight: 600, textTransform: "uppercase", letterSpacing: "0.04em", color: "rgba(0,0,0,0.35)" }}>Max</span><input type="number" value={String(config.max ?? "")} placeholder="100" onChange={(e) => onChange({ ...config, max: e.target.value === "" ? "" : Number(e.target.value) })} style={{ display: "block", width: "100%", marginTop: 4, fontSize: 11, padding: "4px 0", border: "none", borderBottom: "1px solid rgba(0,0,0,0.08)", outline: "none", color: "#374151", backgroundColor: "transparent" }} /></div>
      <div style={{ flex: "1 1 60px" }}><span style={{ fontSize: 9, fontWeight: 600, textTransform: "uppercase", letterSpacing: "0.04em", color: "rgba(0,0,0,0.35)" }}>Schritt</span><input type="number" value={String(config.step ?? "")} placeholder="1" onChange={(e) => onChange({ ...config, step: e.target.value === "" ? "" : Number(e.target.value) })} style={{ display: "block", width: "100%", marginTop: 4, fontSize: 11, padding: "4px 0", border: "none", borderBottom: "1px solid rgba(0,0,0,0.08)", outline: "none", color: "#374151", backgroundColor: "transparent" }} /></div>
      <div style={{ flex: "1 1 60px" }}><span style={{ fontSize: 9, fontWeight: 600, textTransform: "uppercase", letterSpacing: "0.04em", color: "rgba(0,0,0,0.35)" }}>Einheit</span><input type="text" value={String(config.unit ?? "")} onChange={(e) => onChange({ ...config, unit: e.target.value })} placeholder="%" style={{ display: "block", width: "100%", marginTop: 4, fontSize: 11, padding: "4px 0", border: "none", borderBottom: "1px solid rgba(0,0,0,0.08)", outline: "none", color: "#374151", backgroundColor: "transparent" }} /></div>
    </div>
  );
}
    ...
    <div style={{ marginTop: 10 }}>
      <div style={{ marginBottom: 14 }}>
        ...
        <div>
          ...
          <button onClick={() => onChange({ ...config, answers: [...answers, ""] })} style={{ marginTop: 4, display: "flex", alignItems: "center", gap: 4, fontSize: 10, fontWeight: 500, color: "#DC2626", background: "none", border: "none", cursor: "pointer", padding: 0 }}>
            <Plus size={10} strokeWidth={2} />
            Antwort hinzufügen
          </button>
        </div>
...
function ScoringEditor({ question, onUpdate }: { question: Question; onUpdate: (q: Question) => void }) {
  ...
  const inputStyle: React.CSSProperties = { width: 56, fontSize: 11, padding: "3px 6px", border: "none", borderBottom: "1px solid rgba(0,0,0,0.09)", outline: "none", color: "#374151", backgroundColor: "transparent", fontFamily: "inherit", textAlign: "right" as const, MozAppearance: "textfield" as never };
  const labelColStyle: React.CSSProperties = { fontSize: 9, fontWeight: 700, textTransform: "uppercase" as const, letterSpacing: "0.05em", color: "rgba(0,0,0,0.3)", width: 60, textAlign: "right" as const };
            ...
            <>
              ...
              <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 8 }}>
                <div style={{ flex: 1 }}>
                  ...
                  <div style={{ display: "flex", alignItems: "center", gap: 6 }}><span style={{ fontSize: 10, color: "rgba(0,0,0,0.35)" }}>Wert ×</span><input type="number" step="0.1" value={(scoring["__value__"]?.ipp) !== undefined ? String(scoring["__value__"]?.ipp) : ""} onChange={(e) => setWeight("__value__", "ipp", e.target.value)} placeholder="–" style={{ ...inputStyle, width: 70 }} onClick={(e) => e.stopPropagation()} /></div>
                </div>
                <div style={{ flex: 1 }}>
...
function QuestionCard({
  ...
  return (
    <div style={{ position: "relative" }}>
      {dropTarget && (
        <div style={{ position: "absolute", top: -2, left: 0, right: 0, height: 3, borderRadius: 2, backgroundColor: "#3b82f6", opacity: 0.6, transition: "opacity 0.15s ease" }} />
      )}
      <div
      ...
      >
        ...
        <div onClick={onToggle} style={{ display: "flex", alignItems: "center", gap: 8, padding: "0 14px", height: 44, cursor: "pointer", userSelect: "none" }}>
          ...
          <div style={{ width: 18, height: 18, borderRadius: "50%", background: "linear-gradient(to bottom, #DC2626, #e84040)", display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}>
            <span style={{ fontSize: 9, fontWeight: 700, color: "#fff" }}>{index + 1}</span>
          </div>
          <span style={{ fontSize: 9, fontWeight: 600, textTransform: "uppercase", padding: "2px 8px", borderRadius: 4, backgroundColor: badge.bg, color: badge.text, letterSpacing: "0.02em", flexShrink: 0 }}>
          ...
          <div style={{ padding: "0 14px 14px" }}>
            ...
            <div style={{ marginTop: 14 }}>
              <button onClick={(e) => { e.stopPropagation(); setLogicOpen(!logicOpen); }} style={{ display: "flex", alignItems: "center", gap: 7, width: "100%", padding: "8px 0 6px", fontSize: 11, fontWeight: 600, color: question.rules.length > 0 ? "#DC2626" : "rgba(0,0,0,0.35)", background: "none", border: "none", cursor: "pointer", borderTop: "1px solid rgba(0,0,0,0.04)" }}>
                ...
                {question.rules.length > 0 && (
                  <span style={{ fontSize: 9, fontWeight: 700, padding: "1px 7px", borderRadius: 10, backgroundColor: "rgba(220,38,38,0.08)", color: "#DC2626" }}>
              ...
              <div style={{ maxHeight: logicOpen ? 2000 : 0, opacity: logicOpen ? 1 : 0, overflow: "hidden", transition: "max-height 0.3s cubic-bezier(0.4,0,0.2,1), opacity 0.2s ease" }}>
                <ConditionalLogicEditor rules={question.rules} onChange={(newRules) => onUpdate({ ...question, rules: newRules })} allQuestions={allQuestions} currentIndex={index} />
              </div>
            </div>
  ...
  return (
    ...
    >
      ...
      <div style={{ height: 56, borderBottom: "1px solid rgba(0,0,0,0.06)", display: "flex", alignItems: "center", justifyContent: "space-between", padding: "0 24px", flexShrink: 0 }}>
        <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
            <div style={{ width: 22, height: 22, borderRadius: 6, background: "linear-gradient(to bottom, #059669, #047857)", display: "flex", alignItems: "center", justifyContent: "center", boxShadow: "0 1px 4px rgba(5,150,105,0.25)" }}>
              <Sparkles size={12} strokeWidth={2} color="#fff" />
            </div>
            <span style={{ fontSize: 14, fontWeight: 600, color: "#1a1a1a", letterSpacing: "-0.01em" }}>
              Spezialfragen
              {fragebogenName && (
                <span style={{ fontSize: 11, fontWeight: 400, color: "rgba(0,0,0,0.35)", marginLeft: 8 }}>· {fragebogenName}</span>
              )}
            </span>
          </div>
        </div>
      ...
      <div style={{ flex: 1, display: "flex", minHeight: 0 }}>
        {/* Left sidebar */}
        <div style={{ width: 220, backgroundColor: "rgba(0,0,0,0.02)", borderRight: "1px solid rgba(0,0,0,0.06)", padding: "20px 12px", flexShrink: 0, overflowY: "auto", scrollbarWidth: "none" }}>
          {/* Question types */}
          <span style={{ fontSize: 9, fontWeight: 600, textTransform: "uppercase", letterSpacing: "0.06em", color: "rgba(0,0,0,0.35)", padding: "0 6px" }}>
            Fragentypen
          </span>
          <div style={{ marginTop: 10 }}>
                  ...
                  return (
                    ...
                    >
                      <div style={{ display: "flex", alignItems: "center", gap: 6, marginBottom: 4 }}>
                        <span style={{ fontSize: 8, fontWeight: 600, padding: "1px 5px", borderRadius: 3, backgroundColor: badge.bg, color: badge.text, flexShrink: 0 }}>
                          {typeLabel(sq.type)}
                        </span>
                      ...
                      <div style={{ fontSize: 10, color: sq.text ? "#374151" : "rgba(0,0,0,0.3)", fontStyle: sq.text ? "normal" : "italic", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap", marginBottom: 6 }}>
                        {sq.text || "Kein Fragetext"}
                      </div>
                      <button
                        onClick={() => importSpezialfrage(sq)}
                        style={{ display: "flex", alignItems: "center", gap: 4, fontSize: 9, fontWeight: 600, color: "#059669", background: "none", border: "none", cursor: "pointer", padding: 0 }}
        ...
        <div style={{ flex: 1, display: "flex", flexDirection: "column", minWidth: 0 }}>
          {/* Header */}
          <div style={{ padding: "20px 28px 16px", flexShrink: 0, borderBottom: "1px solid rgba(0,0,0,0.04)" }}>
            <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
              <Sparkles size={14} strokeWidth={1.8} color="#059669" />
              <span style={{ fontSize: 13, fontWeight: 600, color: "#1a1a1a", letterSpacing: "-0.01em" }}>
                Spezialfragen
              </span>
              {questions.length > 0 && (
                <span style={{ fontSize: 9, fontWeight: 600, padding: "2px 7px", borderRadius: 10, backgroundColor: "rgba(5,150,105,0.08)", color: "#059669" }}>
          ...
          <div
            ref={listRef}
            style={{ flex: 1, overflowY: "auto", padding: "16px 28px 28px", scrollbarWidth: "none", msOverflowStyle: "none" }}
            onDragOver={(e) => e.preventDefault()}
          >
            {questions.length === 0 && (
              <div style={{ display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", paddingTop: 80, gap: 10 }}>
                <div style={{ width: 44, height: 44, borderRadius: 11, backgroundColor: "rgba(5,150,105,0.06)", display: "flex", alignItems: "center", justifyContent: "center" }}>
            ...
            {questions.length > 0 && dragIdx !== null && (
              <div
                onDragOver={(e) => { e.preventDefault(); e.stopPropagation(); setDropIdx(questions.length); }}
