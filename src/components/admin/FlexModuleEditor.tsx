...
const FLEX_SCORING_TYPES = new Set(["single", "multiple", "numeric"]);
...
function FlexQuestionCard({ question, index, isExpanded, onToggle, onUpdate, onDelete, onDragStart, onDragOver, onDrop, dropTarget, allQuestions }: {
...
export function FlexModuleEditor({ onClose, onSave, existingModule }: FlexModuleEditorProps) {
    ...
    <div style={{ position: "fixed", inset: 0, zIndex: 200, backgroundColor: "#ffffff", display: "flex", flexDirection: "column", animation: "flexModuleEditorIn 0.3s cubic-bezier(0.4,0,0.2,1) both" }}>
      ...
      <div style={{ height: 56, borderBottom: "1px solid rgba(0,0,0,0.06)", display: "flex", alignItems: "center", justifyContent: "space-between", padding: "0 24px", flexShrink: 0 }}>
        ...
        <button
          ...
          style={{ display: "flex", alignItems: "center", gap: 5, padding: "7px 18px", fontSize: 11, fontWeight: 600, color: "#ffffff", background: `linear-gradient(to bottom, ${G}, ${GD})`, border: "none", borderRadius: 7, cursor: "pointer", transition: "all 0.15s ease", letterSpacing: "0.01em", boxShadow: `inset 0 1px 0.6px rgba(255,255,255,0.33), inset 0 -1px 0 rgba(255,255,255,0.15), 0 0 0 1px ${GR}, 0 1px 6px rgba(132,204,22,0.25)` }}
        >
          Speichern
        </button>
      </div>
