        ...
        <div style={{
          ...
          <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 12 }}>
            <button
              type="button"
              onClick={prevMonth}
              style={{ background: "none", border: "none", cursor: "pointer", padding: "2px 6px", borderRadius: 6, color: "rgba(0,0,0,0.45)", fontSize: 16, lineHeight: 1 }}
            >‹</button>
            <span style={{ fontSize: 12, fontWeight: 600, color: "#111", fontFamily: "inherit" }}>
                  ...
                  style={{
                    ...
                    background: isSelected
                      ...
                      : isToday
                        ? "rgba(220,38,38,0.07)"
                        : "transparent",
                    color: isSelected ? "#fff" : isToday ? "#DC2626" : "#111",
                    fontSize: 11, fontWeight: isSelected || isToday ? 600 : 400,
                    cursor: "pointer", fontFamily: "inherit",
                    boxShadow: isSelected
                      ? "inset 0 1px 0.6px rgba(255,255,255,0.33), 0 0 0 1px #a91b1b, 0 1px 4px rgba(180,20,20,0.14)"
                      : "none",
                    transition: "background 0.12s",
                  }}
                  onMouseEnter={(e) => { if (!isSelected) e.currentTarget.style.background = "rgba(0,0,0,0.04)"; }}
                  onMouseLeave={(e) => { if (!isSelected) e.currentTarget.style.background = isToday ? "rgba(220,38,38,0.07)" : "transparent"; }}
                >
                  {d}
                </button>
              );
          ...
          <div style={{ display: "flex", justifyContent: "space-between", marginTop: 10, paddingTop: 8, borderTop: "1px solid rgba(0,0,0,0.05)" }}>
            <button
              type="button"
              onClick={clear}
              style={{ background: "none", border: "none", cursor: "pointer", fontSize: 10, color: "rgba(0,0,0,0.35)", fontFamily: "inherit", fontWeight: 500, padding: "2px 4px" }}
            >
              Löschen
            </button>
            <button
              type="button"
              onClick={goToday}
              style={{ background: "none", border: "none", cursor: "pointer", fontSize: 10, color: "#DC2626", fontFamily: "inherit", fontWeight: 600, padding: "2px 4px" }}
            >
              Heute
            </button>
          </div>
...
const ALL_PLZS = Array.from(
  new Set(TEMP_MARKETS.map((m) => m.address.match(/\d{4}/)?.[0] ?? ""))
)
  .filter(Boolean)
  .sort();
...
type FilterDropdownProps =
  | {
      mode: "multi";
      options: { value: string; label: string; badge?: { bg: string; text: string } }[];
      selected: string[];
  ...
  return (
    <div
      ...
      style={{
        ...
        borderRadius: 10,
        boxShadow:
          "0 4px 20px rgba(0,0,0,0.09), 0 1px 4px rgba(0,0,0,0.05), 0 0 0 1px rgba(0,0,0,0.05)",
        minWidth: 180,
        maxHeight: 220,
        overflowY: "auto",
        scrollbarWidth: "none",
              ...
              >
                <div
                  style={{
                    ...
                    borderRadius: 4,
                    flexShrink: 0,
                    border: checked ? "none" : "1.5px solid rgba(0,0,0,0.15)",
          ...
          {props.options.map((opt) => {
            ...
            return (
              <div
                key={opt.value}
                onClick={() => {
                  props.onSelect(active ? null : opt.value);
                  props.onClose();
                }}
                style={{
                  display: "flex",
                  ...
                  cursor: "pointer",
                  transition: "background-color 0.1s ease",
                  backgroundColor: active ? "rgba(220,38,38,0.04)" : "transparent",
                }}
                onMouseEnter={(e) => {
                  if (!active)
                    e.currentTarget.style.backgroundColor = "rgba(0,0,0,0.025)";
              ...
              >
                ...
                <div>
                  <div style={{ fontSize: 11, color: active ? "#DC2626" : "#374151", fontWeight: active ? 600 : 500 }}>
                    {opt.label}
                  </div>
                  {opt.sub && (
                    <div style={{ fontSize: 9, color: "rgba(0,0,0,0.35)", marginTop: 1 }}>
                      {opt.sub}
                    </div>
                  )}
                </div>
...
// ── Mini question list used in both expanded row and preview modal ──
function QuestionConfigSummary({ question }: { question: import("@/types/fragebogen").Question }) {
  ...
  switch (question.type) {
              ...
              <div key={ai}>
                <div style={{ display: "flex", alignItems: "center", gap: 7, padding: "3px 0" }}>
                  ...
                  {isTriggering && opts.length > 0 && (
                    <span style={{ fontSize: 9, color: "#0d9488", fontWeight: 600, marginLeft: 4 }}>→ Multi</span>
                  )}
                </div>
                {isTriggering && opts.length > 0 && (
                  <div style={{ paddingLeft: 22, marginBottom: 2 }}>
                    {opts.map((opt, oi) => (
                      <div key={oi} style={{ display: "flex", alignItems: "center", gap: 7, padding: "2px 0" }}>
                        <div style={{ width: 8, height: 8, borderRadius: 2, border: "1.5px solid rgba(0,0,0,0.10)", flexShrink: 0 }} />
                        <span style={{ fontSize: 9, color: "rgba(0,0,0,0.35)", fontWeight: 400 }}>{opt}</span>
                      </div>
                    ))}
                  </div>
                )}
    ...
    case "likert": {
      ...
      return (
        <div style={{ fontSize: 9, color: "rgba(0,0,0,0.35)", marginTop: 4, paddingLeft: 30 }}>
          Skala {min || "1"}–{max || "5"}{minL || maxL ? ` (${minL || "…"} → ${maxL || "…"})` : ""}
        </div>
      );
    }
    case "matrix": {
    ...
    case "slider": {
      ...
      return (
        <div style={{ fontSize: 9, color: "rgba(0,0,0,0.35)", marginTop: 4, paddingLeft: 30 }}>
          {String(cfg.min || 0)}–{String(cfg.max || 100)}{unit ? ` ${unit}` : ""} (Schritt: {String(cfg.step || 1)})
        </div>
      );
    }
    case "numeric": {
    ...
    case "photo": {
      ...
      return (
        <div style={{ fontSize: 9, color: "rgba(0,0,0,0.35)", marginTop: 4, paddingLeft: 30, fontStyle: "italic" }}>
          {instr}
        </div>
      );
    }
...
function QuestionMiniList({ module }: { module: Module }) {
  ...
  if (module.questions.length === 0) {
    return (
      <div style={{ fontSize: 10, color: "rgba(0,0,0,0.28)", padding: "8px 0 4px" }}>
      ...
      {module.questions.map((q, idx) => {
        ...
        return (
          <div
            ...
            style={{
              borderRadius: 7,
              backgroundColor: isExpanded ? "rgba(0,0,0,0.025)" : "rgba(0,0,0,0.018)",
              overflow: "hidden",
          ...
          >
            ...
            >
              <span style={{ fontSize: 9, color: "rgba(0,0,0,0.3)", paddingTop: 1, minWidth: 14, flexShrink: 0 }}>
                {idx + 1}
              </span>
              <span style={{
              ...
              <span style={{
                fontSize: 11, color: q.text ? "#374151" : "rgba(0,0,0,0.3)",
              ...
              {q.rules.length > 0 && (
                <Zap size={9} strokeWidth={2} color="#DC2626" style={{ flexShrink: 0, marginTop: 2 }} />
              )}
    ...
    <div style={{ position: "relative", marginBottom: 6 }}>
      {isDropTarget && (
        <div style={{
          ...
          height: 3, borderRadius: 2, backgroundColor: "#3b82f6", opacity: 0.6,
        }} />
      )}
      <div
        ...
        style={{
          ...
          borderBottom: expanded ? "1px solid rgba(0,0,0,0.04)" : "1px solid rgba(0,0,0,0.05)",
      ...
      >
        ...
        <div style={{
          ...
          <div
            ...
            style={{ cursor: "grab", color: "rgba(0,0,0,0.2)", display: "flex", alignItems: "center", flexShrink: 0 }}
          >
            <GripVertical size={14} strokeWidth={1.8} />
          </div>
          <span style={{ fontSize: 12, fontWeight: 500, color: "#374151", flex: 1 }}>
            {module.name || "Unbenanntes Modul"}
          </span>
          <span style={{
            fontSize: 9, fontWeight: 600,
            padding: "2px 7px", borderRadius: 4,
            backgroundColor: "rgba(0,0,0,0.04)", color: "rgba(0,0,0,0.35)",
          }}>
            {module.questions.length} F
          </span>
          {/* Expand chevron */}
          <button
            onClick={onToggle}
            style={{
              ...
              padding: 2, color: "rgba(0,0,0,0.25)", transition: "color 0.15s ease",
          ...
          <button
            ...
            style={{
              background: "none", border: "none", cursor: "pointer",
              padding: 2, color: "rgba(0,0,0,0.2)", transition: "color 0.15s ease",
              display: "flex", alignItems: "center",
            }}
            onMouseEnter={(e) => (e.currentTarget.style.color = "#DC2626")}
      ...
      >
        ...
        <div style={{
          ...
          <div style={{ flex: 1 }}>
            ...
            {module.description && (
              <div style={{ fontSize: 11, color: "rgba(0,0,0,0.45)", marginTop: 4, lineHeight: 1.5 }}>
                {module.description}
              </div>
            )}
  ...
  return (
    <div
      ...
      style={{
        ...
        backgroundColor: "#ffffff",
        borderRadius: 8,
        boxShadow: "0 4px 20px rgba(0,0,0,0.10), 0 1px 4px rgba(0,0,0,0.06), 0 0 0 1px rgba(0,0,0,0.05)",
        padding: "4px",
        minWidth: menuW,
      }}
    >
      <button
        ...
        style={{
          ...
          borderRadius: 5, transition: "background-color 0.1s ease",
          textAlign: "left",
        }}
...
// ── Main component ─────────────────────────────────────────────
export function FragebogenEditor({
  onClose,
  onSave,
  existingFragebogen,
  availableModules,
  ...
  const [selectedMarketIds, setSelectedMarketIds] = useState<Set<string>>(
    new Set(existingFragebogen?.markets.map((m) => m.marketId) ?? [])
  );
  const [scheduleType, setScheduleType] = useState<"always" | "scheduled">(
    existingFragebogen?.scheduleType ?? "always"
  );
  ...
  // expanded module rows in workspace
  ...
  const filteredLibraryModules = availableModules.filter((m) =>
    (m.name || "").toLowerCase().includes(moduleSearch.toLowerCase())
  );
  const filteredMarkets = TEMP_MARKETS.filter((m) => {
    const matchSearch =
      !marketSearch ||
      m.chain.toLowerCase().includes(marketSearch.toLowerCase()) ||
      m.address.toLowerCase().includes(marketSearch.toLowerCase());
    const matchChain = filterChain.length === 0 || filterChain.includes(m.chain);
    const matchPlz =
      filterPlz.length === 0 || filterPlz.some((p) => m.address.includes(p));
    // TODO: filter by market RED Monat assignment when real data is available
    return matchSearch && matchChain && matchPlz;
  });

  // Close on Escape (main editor)
  useEffect(() => {
    const handler = (e: KeyboardEvent) => {
      if (e.key === "Escape" && !previewModule && !contextMenu) onClose();
    };
    window.addEventListener("keydown", handler);
    return () => window.removeEventListener("keydown", handler);
    ...
    <>
      ...
      >
        ...
        <div style={{
          ...
          padding: "0 20px", flexShrink: 0, backgroundColor: "#ffffff",
          boxShadow: "0 1px 4px rgba(0,0,0,0.04)",
        }}>
          <button
            onClick={onClose}
          ...
          <span style={{ fontSize: 14, fontWeight: 600, color: "#1a1a1a", letterSpacing: "-0.01em", flex: 1 }}>
            {existingFragebogen ? "Fragebogen bearbeiten" : "Neuer Fragebogen"}
          </span>
          <button
            onClick={handleSave}
            style={{
        ...
        <div style={{ flex: 1, display: "flex", minHeight: 0 }}>
          ...
          <div style={{
            width: 240, backgroundColor: "rgba(0,0,0,0.02)",
            ...
            <div style={{ position: "relative" }}>
              <Search size={12} strokeWidth={1.8} style={{
            ...
            {filteredLibraryModules.length === 0 ? (
              <div style={{
                ...
                padding: "16px 0", lineHeight: 1.6,
              }}>
                Keine Module gefunden.{"\n"}Erstelle zuerst Module.
              </div>
            ) : (
                ...
                {filteredLibraryModules.map((mod) => {
                  ...
                  return (
                    <div
                      ...
                      onContextMenu={(e) => {
                        e.preventDefault();
                        setContextMenu({ moduleId: mod.id, x: e.clientX, y: e.clientY });
                      }}
                      style={{
                        padding: "8px 12px",
                        borderRadius: 8,
                    ...
                    >
                      <span style={{ fontSize: 11, fontWeight: 500, color: alreadyAdded ? "rgba(0,0,0,0.3)" : "#374151", flex: 1 }}>
                        {mod.name || "Unbenannt"}
                      </span>
                      <span style={{
                        fontSize: 9, fontWeight: 600,
            ...
            <div style={{ marginTop: "auto", fontSize: 9, color: "rgba(0,0,0,0.22)", lineHeight: 1.5, textAlign: "center" }}>
              Klicke zum Hinzufügen · Rechtsklick für Details
            </div>
          </div>
            ...
            <div style={sectionStyle}>
              ...
              <div style={{ marginBottom: 16 }}>
                ...
                <input
                  ...
                  placeholder="z.B. Wöchentlicher Marktcheck"
                  style={inputStyle}
                />
              </div>
              <div>
            ...
            <div style={sectionStyle}>
              ...
              {selectedModules.length === 0 ? (
                ...
                <div>
                  ...
                  {/* invisible final drop zone */}
                  <div
                    style={{ height: 10 }}
                    onDragOver={(e) => { e.preventDefault(); setDropTarget(selectedModules.length); }}
              ...
              <div style={{ display: "flex", gap: 24 }}>
                ...
                <div style={{ flex: 1 }}>
                  <label style={labelStyle}>
                    Märkte{selectedMarketIds.size > 0 ? ` · ${selectedMarketIds.size} ausgewählt` : ""}
                  </label>

                  {/* Search */}
                  <div style={{ position: "relative", marginBottom: 8 }}>
                    <Search size={11} strokeWidth={1.8} style={{
                      position: "absolute", left: 9, top: "50%", transform: "translateY(-50%)",
                      color: "rgba(0,0,0,0.3)", pointerEvents: "none",
                    }} />
                    <input
                  ...
                  <div style={{ display: "flex", alignItems: "center", gap: 6, marginBottom: 8, flexWrap: "wrap" }}>
                    ...
                    <div style={{ position: "relative" }}>
                      <button
                        ...
                        style={{
                          ...
                          padding: "5px 10px", borderRadius: 7, fontSize: 10, fontWeight: 500,
                          border: filterChain.length > 0 ? "1px solid rgba(220,38,38,0.25)" : "1px solid rgba(0,0,0,0.08)",
                          backgroundColor: filterChain.length > 0 ? "rgba(220,38,38,0.04)" : "#fff",
                          color: filterChain.length > 0 ? "#DC2626" : "rgba(0,0,0,0.55)",
                      ...
                      >
                        ...
                        {filterChain.length > 0 && (
                          <span style={{
                            ...
                            backgroundColor: "#DC2626", color: "#fff", lineHeight: 1.4,
                          }}>
                            {filterChain.length}
                          </span>
                        )}
                        <ChevronDown size={10} strokeWidth={2} style={{
                          transition: "transform 0.2s ease",
                          transform: openFilter === "chain" ? "rotate(180deg)" : "rotate(0deg)",
                        }} />
                      </button>
                      {openFilter === "chain" && (
                        <FilterDropdown
                          ...
                          options={ALL_CHAINS.map((c) => {
                            const clr = chainColor(c);
                            return { value: c, label: c, badge: { bg: clr.bg, text: clr.text } };
                          })}
                    ...
                    <div style={{ position: "relative" }}>
                      <button
                        ...
                        style={{
                          display: "flex", alignItems: "center", gap: 5,
                          padding: "5px 10px", borderRadius: 7, fontSize: 10, fontWeight: 500,
                          border: filterPlz.length > 0 ? "1px solid rgba(220,38,38,0.25)" : "1px solid rgba(0,0,0,0.08)",
                          backgroundColor: filterPlz.length > 0 ? "rgba(220,38,38,0.04)" : "#fff",
                          color: filterPlz.length > 0 ? "#DC2626" : "rgba(0,0,0,0.55)",
                          cursor: "pointer", transition: "all 0.15s ease", fontFamily: "inherit",
                        }}
                      >
                        PLZ
                        ...
                        <ChevronDown size={10} strokeWidth={2} style={{
                          transition: "transform 0.2s ease",
                          transform: openFilter === "plz" ? "rotate(180deg)" : "rotate(0deg)",
                        }} />
                      </button>
                      {openFilter === "plz" && (
                        <FilterDropdown
                          mode="multi"
                          options={ALL_PLZS.map((p) => ({ value: p, label: `${p} Wien` }))}
                          selected={filterPlz}
                          onToggle={(v) =>
                            setFilterPlz((prev) =>
                              prev.includes(v) ? prev.filter((x) => x !== v) : [...prev, v]
                    ...
                    <div style={{ position: "relative" }}>
                      <button
                        onClick={() => setOpenFilter(openFilter === "redmonat" ? null : "redmonat")}
                        style={{
                          display: "flex", alignItems: "center", gap: 5,
                          padding: "5px 10px", borderRadius: 7, fontSize: 10, fontWeight: 500,
                          border: filterRedMonat ? "1px solid rgba(220,38,38,0.25)" : "1px solid rgba(0,0,0,0.08)",
                          backgroundColor: filterRedMonat ? "rgba(220,38,38,0.04)" : "#fff",
                          color: filterRedMonat ? "#DC2626" : "rgba(0,0,0,0.55)",
                          cursor: "pointer", transition: "all 0.15s ease", fontFamily: "inherit",
                        }}
                      >
                        ...
                        <ChevronDown size={10} strokeWidth={2} style={{
                          transition: "transform 0.2s ease",
                          transform: openFilter === "redmonat" ? "rotate(180deg)" : "rotate(0deg)",
                        }} />
                      </button>
                      {openFilter === "redmonat" && (
                        <FilterDropdown
                          mode="single"
                          options={RED_MONATS.map((rm) => ({
                            value: rm.label,
                            label: rm.label,
                            sub: `${fmtDate(rm.start)} – ${fmtDate(rm.end)}`,
                          }))}
                          selected={filterRedMonat}
                          onSelect={(v) => setFilterRedMonat(v)}
                          onClose={() => setOpenFilter(null)}
                        />
                      )}
                    </div>

                    {/* Alle auswählen + reset X */}
                    ...
                    {(() => {
                      ...
                      >
                        ...
                        {filteredMarkets.length < TEMP_MARKETS.length && (
                    ...
                    {(filterChain.length > 0 || filterPlz.length > 0 || filterRedMonat) && (
                      <button
                        ...
                        style={{
                          ...
                          alignSelf: "stretch",
                          padding: "0 10px", borderRadius: 7, flexShrink: 0,
                          background: "linear-gradient(to bottom, #1a1a1a, #111111)",
                          boxShadow: "inset 0 1px 0.6px rgba(255,255,255,0.12), inset 0 -1px 0 rgba(255,255,255,0.06), 0 0 0 1px #000, 0 1px 6px rgba(0,0,0,0.22)",
                  ...
                  {(filterChain.length > 0 || filterPlz.length > 0 || filterRedMonat) && (
                    <div style={{
                      ...
                      <span style={{ fontSize: 9, color: "rgba(0,0,0,0.35)", fontWeight: 500, flexShrink: 0 }}>
                        {filteredMarkets.length} / {TEMP_MARKETS.length} Märkte
                      </span>
                      {filterChain.length > 0 && (
                        <button
                          ...
                          style={{
                            ...
                            backgroundColor: "rgba(220,38,38,0.07)", color: "#DC2626",
                            border: "none", cursor: "pointer", fontFamily: "inherit",
                          }}
                        >
                          {filterChain.join(", ")}
                      ...
                      {filterRedMonat && (
                        <button
                          onClick={() => setFilterRedMonat(null)}
                          style={{
                            display: "flex", alignItems: "center", gap: 4,
                            padding: "2px 7px", borderRadius: 5, fontSize: 9, fontWeight: 600,
                            backgroundColor: "rgba(220,38,38,0.07)", color: "#DC2626",
                      ...
                      filteredMarkets.map((m) => {
                        ...
                        return (
                          ...
                          >
                            ...
                            <span style={{ fontSize: 10, color: "rgba(0,0,0,0.45)", flex: 1, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>
                              {m.address}
                            </span>
                          </div>
                        );
                ...
                <div style={{ width: 220, flexShrink: 0 }}>
                  ...
                  <div style={{ display: "flex", gap: 6, marginBottom: 16, width: "100%" }}>
                    <button
                      ...
                      style={{ ...btnBase, flex: 1, textAlign: "center", ...(scheduleType === "always" ? btnActive : btnInactive) }}
                    >
                      Immer aktiv
                    </button>
                    <button
                      onClick={() => setScheduleType("scheduled")}
                      style={{ ...btnBase, flex: 1, textAlign: "center", ...(scheduleType === "scheduled" ? btnActive : btnInactive) }}
                    >
                      Zeitraum
                    </button>
                  </div>
                    ...
                    <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
                      ...
                      {startDate && endDate && (
                        <div style={{
                          ...
                          fontSize: 10, color: "rgba(0,0,0,0.4)", lineHeight: 1.5,
                          textAlign: "center",
                        }}>
                  ...
                  {scheduleType === "always" && (
                    <div style={{
                      ...
                      fontSize: 10, color: "#059669", fontWeight: 500, lineHeight: 1.5,
                    }}>
      ...
      {contextMenu && (
        <ContextMenu
          ...
          y={contextMenu.y}
          onDetails={() => {
            const mod = availableModules.find((m) => m.id === contextMenu.moduleId);
            if (mod) setPreviewModule(mod);
          }}
