...
const Y_BG_FAINT = "rgba(245,158,11,0.04)";

let _kfid = 0;
function nextId(): string {
  _kfid += 1;
  return `kfb-${_kfid}-${Date.now()}`;
}
      ...
      {open && (
        <div style={{
                ...
                <button
                  ...
                  style={{
                    ...
                    background: isSelected
                      ? `linear-gradient(to bottom, ${Y}, ${YD})`
                      : isToday ? Y_BG : "transparent",
                    color: isSelected ? "#fff" : isToday ? YD : "#111",
                    fontSize: 11, fontWeight: isSelected || isToday ? 600 : 400,
                    cursor: "pointer", fontFamily: "inherit",
                    boxShadow: isSelected
                      ? `inset 0 1px 0.6px rgba(255,255,255,0.33), 0 0 0 1px ${YR}, 0 1px 4px rgba(245,158,11,0.25)`
                      : "none",
                    transition: "background 0.12s",
                  }}
                  onMouseEnter={(e) => { if (!isSelected) e.currentTarget.style.background = "rgba(0,0,0,0.04)"; }}
                  onMouseLeave={(e) => { if (!isSelected) e.currentTarget.style.background = isToday ? Y_BG : "transparent"; }}
                >
                  {d}
                </button>
              );
          ...
          <div style={{ display: "flex", justifyContent: "space-between", marginTop: 10, paddingTop: 8, borderTop: "1px solid rgba(0,0,0,0.05)" }}>
            ...
            <button type="button" onClick={goToday} style={{ background: "none", border: "none", cursor: "pointer", fontSize: 10, color: YD, fontFamily: "inherit", fontWeight: 600, padding: "2px 4px" }}>Heute</button>
          </div>
        </div>
      )}
    </div>
      ...
      {props.mode === "multi" ? (
        <>
          {props.options.map((opt) => {
            ...
            return (
              <div key={opt.value} onClick={() => props.onToggle(opt.value)} style={{ display: "flex", alignItems: "center", gap: 9, padding: "7px 14px", cursor: "pointer", transition: "background-color 0.1s ease" }} onMouseEnter={(e) => (e.currentTarget.style.backgroundColor = "rgba(0,0,0,0.025)")} onMouseLeave={(e) => (e.currentTarget.style.backgroundColor = "transparent")}>
                <div style={{ width: 14, height: 14, borderRadius: 4, flexShrink: 0, border: checked ? "none" : "1.5px solid rgba(0,0,0,0.15)", backgroundColor: checked ? YD : "transparent", display: "flex", alignItems: "center", justifyContent: "center", transition: "all 0.15s ease" }}>
          ...
          {props.selected.length > 0 && (
            <div style={{ borderTop: "1px solid rgba(0,0,0,0.05)", padding: "6px 14px 2px", display: "flex", justifyContent: "flex-end" }}>
              <button onClick={props.onClear} style={{ background: "none", border: "none", cursor: "pointer", fontSize: 10, color: "rgba(0,0,0,0.35)", fontWeight: 500, padding: "2px 0", fontFamily: "inherit" }} onMouseEnter={(e) => (e.currentTarget.style.color = YD)} onMouseLeave={(e) => (e.currentTarget.style.color = "rgba(0,0,0,0.35)")}>Zurücksetzen</button>
            </div>
          )}
        </>
      ) : (
        <>
          {props.options.map((opt) => {
            ...
            return (
              <div key={opt.value} onClick={() => { props.onSelect(active ? null : opt.value); props.onClose(); }} style={{ display: "flex", alignItems: "center", gap: 9, padding: "7px 14px", cursor: "pointer", transition: "background-color 0.1s ease", backgroundColor: active ? Y_BG_FAINT : "transparent" }} onMouseEnter={(e) => { if (!active) e.currentTarget.style.backgroundColor = "rgba(0,0,0,0.025)"; }} onMouseLeave={(e) => { e.currentTarget.style.backgroundColor = active ? Y_BG_FAINT : "transparent"; }}>
                <div style={{ width: 14, height: 14, borderRadius: "50%", flexShrink: 0, border: active ? "none" : "1.5px solid rgba(0,0,0,0.15)", backgroundColor: active ? YD : "transparent", display: "flex", alignItems: "center", justifyContent: "center", transition: "all 0.15s ease" }}>
                  {active && <Check size={7} strokeWidth={3} color="#fff" />}
                </div>
                <div>
                  <div style={{ fontSize: 11, color: active ? YD : "#374151", fontWeight: active ? 600 : 500 }}>{opt.label}</div>
          ...
          {props.selected && (
            <div style={{ borderTop: "1px solid rgba(0,0,0,0.05)", padding: "6px 14px 2px", display: "flex", justifyContent: "flex-end" }}>
              <button onClick={() => { props.onSelect(null); props.onClose(); }} style={{ background: "none", border: "none", cursor: "pointer", fontSize: 10, color: "rgba(0,0,0,0.35)", fontWeight: 500, padding: "2px 0", fontFamily: "inherit" }} onMouseEnter={(e) => (e.currentTarget.style.color = YD)} onMouseLeave={(e) => (e.currentTarget.style.color = "rgba(0,0,0,0.35)")}>Zurücksetzen</button>
            </div>
          )}
        </>
      )}
        ...
        return (
          <div key={q.id} style={{ borderRadius: 7, backgroundColor: isExpanded ? "rgba(0,0,0,0.025)" : "rgba(0,0,0,0.018)", overflow: "hidden", transition: "background-color 0.15s ease" }}>
            <div onClick={() => toggle(q.id)} style={{ display: "flex", alignItems: "flex-start", gap: 8, padding: "6px 10px", cursor: "pointer", userSelect: "none" }}>
              ...
              {q.rules.length > 0 && <Zap size={9} strokeWidth={2} color={YD} style={{ flexShrink: 0, marginTop: 2 }} />}
...
function ModuleRow({ module, index, onRemove, onDragStart, onDragOver, onDrop, isDropTarget, expanded, onToggle }: {
  ...
  return (
    <div style={{ position: "relative", marginBottom: 6 }}>
      {isDropTarget && <div style={{ position: "absolute", top: -2, left: 0, right: 0, height: 3, borderRadius: 2, backgroundColor: YD, opacity: 0.6 }} />}
      <div onDragOver={(e) => { e.preventDefault(); onDragOver(index); }} onDrop={(e) => { e.preventDefault(); onDrop(); }} style={{ borderRadius: expanded ? "8px 8px 0 0" : 8, border: "1px solid rgba(0,0,0,0.05)", borderBottom: expanded ? "1px solid rgba(0,0,0,0.04)" : "1px solid rgba(0,0,0,0.05)", backgroundColor: "#fafafa", overflow: "hidden" }}>
        <div style={{ display: "flex", alignItems: "center", gap: 8, padding: "9px 12px" }}>
          ...
          <button onClick={onRemove} style={{ background: "none", border: "none", cursor: "pointer", padding: 2, color: "rgba(0,0,0,0.2)", transition: "color 0.15s ease", display: "flex", alignItems: "center" }} onMouseEnter={(e) => (e.currentTarget.style.color = YD)} onMouseLeave={(e) => (e.currentTarget.style.color = "rgba(0,0,0,0.2)")}>
        ...
        <div style={{ padding: "18px 20px 14px", borderBottom: "1px solid rgba(0,0,0,0.06)", flexShrink: 0, display: "flex", alignItems: "flex-start", gap: 12 }}>
          <div style={{ flex: 1 }}>
            ...
            <div style={{ display: "flex", alignItems: "center", gap: 8, marginTop: 8 }}>
              <span style={{ fontSize: 9, fontWeight: 600, padding: "2px 8px", borderRadius: 4, backgroundColor: Y_BG, color: YD }}>{module.questions.length} {module.questions.length === 1 ? "Frage" : "Fragen"}</span>
          ...
          <button onClick={onClose} style={{ background: "none", border: "none", cursor: "pointer", padding: 4, color: "rgba(0,0,0,0.35)", display: "flex", alignItems: "center", transition: "color 0.15s ease", flexShrink: 0 }} onMouseEnter={(e) => (e.currentTarget.style.color = YD)} onMouseLeave={(e) => (e.currentTarget.style.color = "rgba(0,0,0,0.35)")}>
...
// ── Main Component ─────────────────────────────────────────────
export function KuehlerFragebogenEditor({
  onClose,
  onSave,
  existingFragebogen,
  availableModules,
  ...
  const btnActive: React.CSSProperties = { background: `linear-gradient(to bottom, ${Y}, ${YD})`, color: "#fff", boxShadow: `inset 0 1px 0.6px rgba(255,255,255,0.33), inset 0 -1px 0 rgba(255,255,255,0.15), 0 0 0 1px ${YR}, 0 1px 6px rgba(245,158,11,0.25)` };
    ...
    <>
      <div style={{ position: "fixed", inset: 0, zIndex: 200, backgroundColor: "#ffffff", display: "flex", flexDirection: "column" }}>
        ...
        <div style={{ height: 56, borderBottom: "1px solid rgba(0,0,0,0.07)", display: "flex", alignItems: "center", gap: 12, padding: "0 20px", flexShrink: 0, backgroundColor: "#ffffff", boxShadow: "0 1px 4px rgba(0,0,0,0.04)" }}>
          <button onClick={onClose} style={{ background: "none", border: "none", cursor: "pointer", padding: 4, color: "rgba(0,0,0,0.4)", display: "flex", alignItems: "center", transition: "color 0.15s ease" }} onMouseEnter={(e) => (e.currentTarget.style.color = YD)} onMouseLeave={(e) => (e.currentTarget.style.color = "rgba(0,0,0,0.4)")}>
          ...
          <span style={{ fontSize: 14, fontWeight: 600, color: "#1a1a1a", letterSpacing: "-0.01em", flex: 1 }}>
            {existingFragebogen ? "Kühlerinventur-Fragebogen bearbeiten" : "Neuer Kühlerinventur-Fragebogen"}
          </span>
                ...
                {filteredLibraryModules.map((mod) => {
                  ...
                  return (
                    <div
                      ...
                      style={{ padding: "8px 12px", borderRadius: 8, border: alreadyAdded ? `1px solid ${Y_BG}` : "1px solid rgba(0,0,0,0.06)", backgroundColor: alreadyAdded ? Y_BG_FAINT : "#ffffff", cursor: alreadyAdded ? "default" : "pointer", transition: "all 0.15s ease", display: "flex", alignItems: "center", gap: 8, userSelect: "none" }}
          ...
          <div style={{ flex: 1, overflowY: "auto", backgroundColor: "#f5f5f7", padding: "24px 32px" }}>
            ...
            <div style={sectionStyle}>
              ...
              <div style={{ marginBottom: 16 }}>
                ...
                <input type="text" value={name} onChange={(e) => setName(e.target.value)} placeholder="z.B. Wöchentlicher Kühlercheck" style={inputStyle} />
              </div>
              <div>
              ...
              <div style={{ marginTop: 14, display: "flex", alignItems: "center", justifyContent: "space-between" }}>
                ...
                <button onClick={() => setNurEinmal((v) => !v)} style={{ width: 38, height: 22, borderRadius: 99, border: "none", cursor: "pointer", flexShrink: 0, background: nurEinmal ? YD : "rgba(0,0,0,0.1)", transition: "all 0.18s ease", position: "relative" }}>
            ...
            <div style={sectionStyle}>
              <div style={sectionHeadingStyle}>
                Module
                <span style={{ marginLeft: 8, fontSize: 9, fontWeight: 600, padding: "2px 7px", borderRadius: 4, backgroundColor: Y_BG, color: YD }}>{selectedModules.length}</span>
              </div>
              ...
              {selectedModules.length > 0 && (
                <button onClick={() => {}} style={{ marginTop: 8, display: "flex", alignItems: "center", gap: 5, fontSize: 10, fontWeight: 500, color: YD, background: "none", border: "none", cursor: "pointer", padding: 0 }}>
              ...
              <div style={{ display: "flex", gap: 24 }}>
                ...
                <div style={{ flex: 1 }}>
                  ...
                  <div style={{ display: "flex", alignItems: "center", gap: 6, marginBottom: 8, flexWrap: "wrap" }}>
                    {/* Handelskette */}
                    <div style={{ position: "relative" }}>
                      <button onClick={() => setOpenFilter(openFilter === "chain" ? null : "chain")} style={{ display: "flex", alignItems: "center", gap: 5, padding: "5px 10px", borderRadius: 7, fontSize: 10, fontWeight: 500, border: filterChain.length > 0 ? `1px solid rgba(245,158,11,0.25)` : "1px solid rgba(0,0,0,0.08)", backgroundColor: filterChain.length > 0 ? Y_BG_FAINT : "#fff", color: filterChain.length > 0 ? YD : "rgba(0,0,0,0.55)", cursor: "pointer", transition: "all 0.15s ease", fontFamily: "inherit" }}>
                        Handelskette
                        {filterChain.length > 0 && <span style={{ fontSize: 8, fontWeight: 700, padding: "1px 5px", borderRadius: 10, backgroundColor: YD, color: "#fff", lineHeight: 1.4 }}>{filterChain.length}</span>}
                    ...
                    {/* PLZ */}
                    <div style={{ position: "relative" }}>
                      <button onClick={() => setOpenFilter(openFilter === "plz" ? null : "plz")} style={{ display: "flex", alignItems: "center", gap: 5, padding: "5px 10px", borderRadius: 7, fontSize: 10, fontWeight: 500, border: filterPlz.length > 0 ? `1px solid rgba(245,158,11,0.25)` : "1px solid rgba(0,0,0,0.08)", backgroundColor: filterPlz.length > 0 ? Y_BG_FAINT : "#fff", color: filterPlz.length > 0 ? YD : "rgba(0,0,0,0.55)", cursor: "pointer", transition: "all 0.15s ease", fontFamily: "inherit" }}>
                        PLZ
                        {filterPlz.length > 0 && <span style={{ fontSize: 8, fontWeight: 700, padding: "1px 5px", borderRadius: 10, backgroundColor: YD, color: "#fff", lineHeight: 1.4 }}>{filterPlz.length}</span>}
                      ...
                      {(() => {
                        ...
                        return (
                          <button
                            ...
                            style={{ display: "flex", alignItems: "center", gap: 5, padding: "5px 12px", borderRadius: 7, fontSize: 10, fontWeight: 600, background: `linear-gradient(to bottom, ${Y}, ${YD})`, color: "#fff", boxShadow: `inset 0 1px 0.6px rgba(255,255,255,0.33), inset 0 -1px 0 rgba(255,255,255,0.15), 0 0 0 1px ${YR}, 0 1px 6px rgba(245,158,11,0.25)`, border: "none", cursor: "pointer", fontFamily: "inherit", transition: "opacity 0.15s ease" }}
                  ...
                  {(filterChain.length > 0 || filterPlz.length > 0 || filterRedMonat) && (
                    <div style={{ display: "flex", alignItems: "center", gap: 6, flexWrap: "wrap", marginBottom: 8, padding: "6px 10px", borderRadius: 8, backgroundColor: Y_BG_FAINT }}>
                      ...
                      {filterChain.length > 0 && <button onClick={() => setFilterChain([])} style={{ display: "flex", alignItems: "center", gap: 4, padding: "2px 7px", borderRadius: 5, fontSize: 9, fontWeight: 600, backgroundColor: Y_BG, color: YD, border: "none", cursor: "pointer", fontFamily: "inherit" }}>{filterChain.join(", ")}<X size={8} strokeWidth={2.5} /></button>}
                      {filterPlz.length > 0 && <button onClick={() => setFilterPlz([])} style={{ display: "flex", alignItems: "center", gap: 4, padding: "2px 7px", borderRadius: 5, fontSize: 9, fontWeight: 600, backgroundColor: Y_BG, color: YD, border: "none", cursor: "pointer", fontFamily: "inherit" }}>PLZ: {filterPlz.join(", ")}<X size={8} strokeWidth={2.5} /></button>}
                      {filterRedMonat && <button onClick={() => setFilterRedMonat(null)} style={{ display: "flex", alignItems: "center", gap: 4, padding: "2px 7px", borderRadius: 5, fontSize: 9, fontWeight: 600, backgroundColor: Y_BG, color: YD, border: "none", cursor: "pointer", fontFamily: "inherit" }}>{filterRedMonat}<X size={8} strokeWidth={2.5} /></button>}
                    </div>
                  )}
                ...
                <div style={{ width: 220, flexShrink: 0 }}>
                  ...
                  {scheduleType === "scheduled" && (
                    <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
                      ...
                      {startDate && endDate && (
                        <div style={{ padding: "8px 12px", borderRadius: 8, backgroundColor: Y_BG_FAINT, fontSize: 10, color: "rgba(0,0,0,0.4)", lineHeight: 1.5, textAlign: "center" }}>
