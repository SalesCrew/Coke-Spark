...
const GD = "#65a30d";
const GR = "#4d7c0f";
const G_BG = "rgba(132,204,22,0.06)";
const G_BG_HVR = "rgba(132,204,22,0.12)";
const G_BORDER = "rgba(132,204,22,0.25)";
const G_BORDER_HVR = "rgba(132,204,22,0.4)";

let _fxqid = 0;
function nextId(): string {
  _fxqid += 1;
  return `fxq-${_fxqid}-${Date.now()}`;
}
...
function FlexCLDropdown({ value, displayText, options, onChange, placeholder }: {
...
function FlexConditionalLogicEditor({ rules, onChange, allQuestions, currentIndex }: {
        ...
        return (
          <div key={rule.id} style={{ border: "1px solid rgba(0,0,0,0.05)", borderRadius: 10, padding: "12px 14px 14px", marginBottom: 8, backgroundColor: "#fff" }}>
            ...
            <div style={{ display: "flex", alignItems: "center", marginBottom: 8 }}>
              ...
              <FlexCLDropdown value={rule.triggerQuestionId} options={triggerOptions} onChange={(v) => updateRule(rule.id, { triggerQuestionId: v, triggerValue: "", triggerValueMax: "" })} placeholder="Frage wählen..." />
            </div>
            <div style={{ display: "flex", alignItems: "center", marginBottom: 8 }}>
              <span style={clFieldLabel}>Operator</span>
              <FlexCLDropdown value={rule.operator} options={operatorOptions} onChange={(v) => updateRule(rule.id, { operator: v, triggerValue: v === "between" ? "" : rule.triggerValue, triggerValueMax: "" })} />
            </div>
            <div style={{ display: "flex", alignItems: "center", marginBottom: 8 }}>
              ...
              {isBetween ? (
                ...
                <FlexCLDropdown value={rule.triggerValue} options={answerOptions} onChange={(v) => updateRule(rule.id, { triggerValue: v })} placeholder="Antwort wählen..." />
              ) : (
                <div style={{ flex: 1 }}>
            ...
            <div style={{ display: "flex", alignItems: "center", marginBottom: 10 }}>
              ...
              <FlexCLDropdown value={rule.action} options={actionOptions} onChange={(v) => updateRule(rule.id, { action: v as "hide" | "show" })} />
            </div>
            <div>
...
function FlexToggle({ value, onChange }: { value: boolean; onChange: (v: boolean) => void }) {
  return (
    <button
...
function FlexLikertConfig({ config, onChange }: { config: Record<string, unknown>; onChange: (c: Record<string, unknown>) => void }) {
...
function FlexNumericConfig({ config, onChange }: { config: Record<string, unknown>; onChange: (c: Record<string, unknown>) => void }) {
  const iStyle = { display: "block", width: "100%", marginTop: 4, fontSize: 11, padding: "4px 0", border: "none", borderBottom: "1px solid rgba(0,0,0,0.08)", outline: "none", color: "#374151", backgroundColor: "transparent" };
  const lStyle = { fontSize: 9, fontWeight: 600, textTransform: "uppercase" as const, letterSpacing: "0.04em", color: "rgba(0,0,0,0.35)" };
  return (
    <div style={{ marginTop: 10, display: "flex", gap: 12, alignItems: "flex-end", flexWrap: "wrap" }}>
      ...
      <div style={{ display: "flex", alignItems: "center", gap: 6, paddingBottom: 4 }}>
        <FlexToggle value={!!config.decimals} onChange={(v) => onChange({ ...config, decimals: v })} />
...
function FlexSliderConfig({ config, onChange }: { config: Record<string, unknown>; onChange: (c: Record<string, unknown>) => void }) {
  const iStyle = { display: "block", width: "100%", marginTop: 4, fontSize: 11, padding: "4px 0", border: "none", borderBottom: "1px solid rgba(0,0,0,0.08)", outline: "none", color: "#374151", backgroundColor: "transparent" };
  const lStyle = { fontSize: 9, fontWeight: 600, textTransform: "uppercase" as const, letterSpacing: "0.04em", color: "rgba(0,0,0,0.35)" };
  return (
    <div style={{ marginTop: 10, display: "flex", gap: 12, flexWrap: "wrap" }}>
...
function FlexPhotoConfig({ config, onChange }: { config: Record<string, unknown>; onChange: (c: Record<string, unknown>) => void }) {
  return <PhotoTagsConfig config={config} onChange={onChange} accentColor="#84CC16" />;
}

function FlexListEditor({ label, items, onChange }: { label: string; items: string[]; onChange: (items: string[]) => void }) {
  return (
    <div style={{ flex: 1 }}>
...
function FlexMatrixConfig({ config, onChange }: { config: Record<string, unknown>; onChange: (c: Record<string, unknown>) => void }) {
  ...
  return (
    <div style={{ marginTop: 10 }}>
      ...
      <div style={{ display: "flex", gap: 20 }}>
        <FlexListEditor label="Zeilen" items={rows} onChange={(r) => onChange({ ...config, rows: r })} />
        <FlexListEditor label="Spalten" items={columns} onChange={(c) => onChange({ ...config, columns: c })} />
      </div>
    </div>
  );
}

function FlexYesNoMultiConfig({ config, onChange }: { config: Record<string, unknown>; onChange: (c: Record<string, unknown>) => void }) {
    ...
    <div style={{ marginTop: 10 }}>
      ...
      {orderedBranches.map(({ ans, branch }, idx) => (
        <div key={ans} style={{ marginBottom: 12 }}>
          ...
          <FlexChoiceConfig options={branch.options} onChange={(opts) => updateBranchOptions(ans, opts)} />
        </div>
      ))}
    </div>
  );
...
function FlexTypeConfig({ question, onUpdate }: { question: Question; onUpdate: (q: Question) => void }) {
  ...
  switch (question.type) {
    ...
    case "numeric": return <FlexNumericConfig config={question.config} onChange={setConfig} />;
    case "slider": return <FlexSliderConfig config={question.config} onChange={setConfig} />;
    case "photo": return <FlexPhotoConfig config={question.config} onChange={setConfig} />;
    case "matrix": return <FlexMatrixConfig config={question.config} onChange={setConfig} />;
    default: return null;
  }
}
...
const FLEX_SCORING_TYPES = new Set(["single", "multiple", "yesno", "yesnomulti", "likert", "numeric", "slider"]);

function FlexScoringEditor({ question, onUpdate }: { question: Question; onUpdate: (q: Question) => void }) {
  const [open, setOpen] = useState(false);
  const scoring = question.scoring ?? {};

  if (!FLEX_SCORING_TYPES.has(question.type)) return null;
  ...
  return (
    <div style={{ marginTop: 14 }}>
      ...
      >
        ...
        {hasIPP && (
          <span style={{ fontSize: 9, fontWeight: 700, padding: "1px 7px", borderRadius: 10, backgroundColor: G_BG, color: GD }}>IPP</span>
        )}
        {hasBoni && (
          ...
          {!isNumericFactor && (
            <>
              ...
              {scoringOptions.length === 0 && (
                <span style={{ fontSize: 10, color: "rgba(0,0,0,0.28)", fontStyle: "italic" }}>Zuerst Optionen hinzufügen.</span>
              )}
            </>
          )}
          {isNumericFactor && (
...
function FlexImagePill({ src, index, onRemove }: { src: string; index: number; onRemove: () => void }) {
...
function FlexImageAttachment({ value, onChange }: { value: string[]; onChange: (v: string[]) => void }) {
  ...
  return (
    <div style={{ marginTop: 8, display: "flex", alignItems: "center", flexWrap: "wrap", gap: 5 }}>
      ...
      {value.map((src, i) => <FlexImagePill key={i} src={src} index={i} onRemove={() => onChange(value.filter((_, j) => j !== i))} />)}
      {value.length === 0 ? (
...
function FlexQuestionCard({ question, index, isExpanded, onToggle, onUpdate, onDelete, onDragStart, onDragOver, onDrop, onContextTypeMenu, dropTarget, allQuestions, availableChains }: {
      ...
      >
        ...
        <div style={{ maxHeight: isExpanded ? 2000 : 0, opacity: isExpanded ? 1 : 0, overflow: "hidden", transition: "max-height 0.25s cubic-bezier(0.4,0,0.2,1), opacity 0.2s ease" }}>
          <div style={{ padding: "0 14px 14px" }}>
            ...
            <FlexImageAttachment value={(question.config.images as string[]) ?? []} onChange={(v) => onUpdate({ ...question, config: { ...question.config, images: v } })} />
            ...
            <FlexTypeConfig question={question} onUpdate={onUpdate} />
            ...
            <div style={{ marginTop: 14 }}>
              ...
              <div style={{ maxHeight: logicOpen ? 2000 : 0, opacity: logicOpen ? 1 : 0, overflow: "hidden", transition: "max-height 0.3s cubic-bezier(0.4,0,0.2,1), opacity 0.2s ease" }}>
                <FlexConditionalLogicEditor rules={question.rules} onChange={(newRules) => onUpdate({ ...question, rules: newRules })} allQuestions={allQuestions} currentIndex={index} />
              </div>
            </div>
...
interface FlexModuleEditorProps {
  onClose: () => void;
...
export function FlexModuleEditor({ onClose, onSave, existingModule, availableChains = [], existingQuestions = [] }: FlexModuleEditorProps) {
  ...
  return (
    <div style={{ position: "fixed", inset: 0, zIndex: 200, backgroundColor: "#ffffff", display: "flex", flexDirection: "column", animation: "flexModuleEditorIn 0.3s cubic-bezier(0.4,0,0.2,1) both" }}>
      <style>{`
        @keyframes flexModuleEditorIn { from { opacity: 0; transform: scale(0.98); } to { opacity: 1; transform: scale(1); } }
      ...
      <div style={{ height: 56, borderBottom: "1px solid rgba(0,0,0,0.06)", display: "flex", alignItems: "center", justifyContent: "space-between", padding: "0 24px", flexShrink: 0 }}>
        ...
        <button
          onClick={async () => {
            if (isSaving) return;
            const mod: Module = {
              id: existingModule?.id ?? `fxmod-${Date.now()}`,
          ...
          disabled={isSaving}
          style={{ display: "flex", alignItems: "center", gap: 5, padding: "7px 18px", fontSize: 11, fontWeight: 600, color: "#ffffff", background: `linear-gradient(to bottom, ${G}, ${GD})`, border: "none", borderRadius: 7, cursor: isSaving ? "not-allowed" : "pointer", opacity: isSaving ? 0.8 : 1, transition: "all 0.15s ease", letterSpacing: "0.01em", boxShadow: `inset 0 1px 0.6px rgba(255,255,255,0.33), inset 0 -1px 0 rgba(255,255,255,0.15), 0 0 0 1px ${GR}, 0 1px 6px rgba(132,204,22,0.25)` }}
        >
