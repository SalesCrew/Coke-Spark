...
function SmModuleCard({
  ...
  return (
    <div className="sm-module-card" onContextMenu={(event) => { event.preventDefault(); setMenuOpen(true); }}>
      {menuOpen ? (
        <div ref={menuRef} className="sm-context-menu">
          <button type="button" onClick={() => { onDuplicate(); setMenuOpen(false); }}><FileText size={12} />Duplizieren</button>
          <div />
          <button type="button" className="danger" onClick={() => { onDelete(); setMenuOpen(false); }}><Trash2 size={12} />Löschen</button>
        </div>
      ) : null}
      <div
