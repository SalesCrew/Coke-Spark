...
- Every SM question may carry one optional short `config.subheading`; discrete primary answers and matrix-column answers use the position-aligned `config.answerSubheadings` array, and Ja/Nein-Multi branches may carry their own position-aligned `answerSubheadings`. These fields are SM-only, limited to 500 characters each and stored inside the already-versioned JSON configuration. They therefore flow through published question versions, visit snapshots, offline payload caching and historical submissions without a schema migration or any GM data dependency. On the phone, question subheadings use a thin 9 px, low-opacity, two-line treatment; answer subheadings use a 7 px, low-opacity, single-line treatment so empty or populated subtext does not materially change the questionnaire layout.
...
OOS semantics can be authored on `Ja / Nein`, `Ja / Nein Multi`, `Single Choice` and `Multiple Choice` questions. The author assigns the semantic outcome to each visible answer option; the option label itself is never interpreted. Several options may intentionally carry the same outcome, for example both `Behoben` and `Teilweise behoben` may carry `resolved` when they should make the same contribution. Even if several equally tagged options are selected in a Multiple-Choice answer, the question contributes only one detection or remediation result.

For a contradictory Multiple-Choice selection, aggregation is deterministic and conservative. `oos_present` takes precedence over `oos_absent`; for remediation `not_resolved` takes precedence over `partially_resolved`, which takes precedence over `resolved`. This prevents a simultaneously selected positive option from overstating remediation. Editing an option label keeps its mapping when it is a direct in-place rename, while deleting an option removes its mapping. The backend additionally rejects stale-only mappings, duplicate option labels on OOS questions, unsupported question types and outcomes that do not match the configured detection/remediation role.
