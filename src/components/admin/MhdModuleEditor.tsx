...
const P_BG_HVR = "rgba(124,58,237,0.12)";
const P_BORDER = "rgba(124,58,237,0.25)";
const P_BORDER_HVR = "rgba(124,58,237,0.4)";

let _mqid = 0;
function nextId(): string {
  _mqid += 1;
  return `mq-${_mqid}-${Date.now()}`;
}
...
function CLDropdown({ value, displayText, options, onChange, placeholder }: {
        ...
        <div style={{ position: "absolute", top: "calc(100% + 4px)", left: 0, right: 0, zIndex: 50, backgroundColor: "#fff", borderRadius: 8, border: "1px solid rgba(0,0,0,0.06)", boxShadow: "0 4px 16px rgba(0,0,0,0.08), 0 1px 3px rgba(0,0,0,0.04)", padding: "4px", maxHeight: 180, overflowY: "auto" }}>
          {options.map((opt) => (
            <button
              ...
              style={{ width: "100%", display: "flex", alignItems: "center", justifyContent: "space-between", fontSize: 11, fontWeight: value === opt.value ? 600 : 400, color: value === opt.value ? PD : "#374151", padding: "6px 8px", borderRadius: 5, border: "none", cursor: "pointer", backgroundColor: value === opt.value ? P_BG : "transparent", textAlign: "left", transition: "background-color 0.1s ease" }}
              onMouseEnter={(e) => { if (value !== opt.value) e.currentTarget.style.backgroundColor = "rgba(0,0,0,0.025)"; }}
              onMouseLeave={(e) => { e.currentTarget.style.backgroundColor = value === opt.value ? P_BG : "transparent"; }}
            >
              <span style={{ overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap", flex: 1 }}>{opt.label}</span>
              {value === opt.value && <Check size={12} strokeWidth={2.5} color={PD} style={{ flexShrink: 0, marginLeft: 6 }} />}
            </button>
          ))}
        </div>
      )}
...
function ConditionalLogicEditor({ rules, onChange, allQuestions, currentIndex }: {
  ...
  return (
    <div style={{ paddingTop: 6 }}>
        ...
        return (
          <div key={rule.id} style={{ border: "1px solid rgba(0,0,0,0.05)", borderRadius: 10, padding: "12px 14px 14px", marginBottom: 8, backgroundColor: "#fff" }}>
            <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 12 }}>
              ...
              <button onClick={() => removeRule(rule.id)} style={{ background: "none", border: "none", cursor: "pointer", padding: 2, color: "rgba(0,0,0,0.2)", transition: "color 0.15s ease" }} onMouseEnter={(e) => (e.currentTarget.style.color = PD)} onMouseLeave={(e) => (e.currentTarget.style.color = "rgba(0,0,0,0.2)")}>
                  ...
                  {targetCandidates.map((tq) => {
                    ...
                    return (
                      <button key={tq.id} onClick={() => toggleTarget(rule.id, tq.id)}
                        style={{ display: "flex", alignItems: "center", gap: 8, fontSize: 11, fontWeight: 400, padding: "6px 8px", borderRadius: 6, border: "none", cursor: "pointer", transition: "background-color 0.12s ease", backgroundColor: isSelected ? P_BG : "transparent", color: "#374151", textAlign: "left", width: "100%" }}
                        onMouseEnter={(e) => { if (!isSelected) e.currentTarget.style.backgroundColor = "rgba(0,0,0,0.02)"; }}
                        onMouseLeave={(e) => { e.currentTarget.style.backgroundColor = isSelected ? P_BG : "transparent"; }}
                      >
                        <div style={{ width: 16, height: 16, borderRadius: 4, border: isSelected ? "none" : "1.5px solid rgba(0,0,0,0.12)", backgroundColor: isSelected ? PD : "transparent", display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0, transition: "all 0.12s ease" }}>
      ...
      <button
        ...
        style={{ display: "flex", alignItems: "center", justifyContent: "center", gap: 5, width: "100%", fontSize: 10, fontWeight: 600, color: PD, backgroundColor: P_BG, border: `1px dashed ${P_BORDER}`, borderRadius: 8, cursor: "pointer", padding: "8px 0", transition: "all 0.15s ease" }}
        onMouseEnter={(e) => { e.currentTarget.style.backgroundColor = P_BG_HVR; e.currentTarget.style.borderColor = P_BORDER_HVR; }}
        onMouseLeave={(e) => { e.currentTarget.style.backgroundColor = P_BG; e.currentTarget.style.borderColor = P_BORDER; }}
      >
...
function Toggle({ value, onChange }: { value: boolean; onChange: (v: boolean) => void }) {
  return (
    <button
      ...
      style={{ width: 32, height: 18, borderRadius: 9, backgroundColor: value ? PD : "rgba(0,0,0,0.12)", border: "none", cursor: "pointer", position: "relative", transition: "background-color 0.2s ease", flexShrink: 0 }}
    >
      ...
      <div style={{ marginTop: 6 }}>
        {options.map((opt, i) => (
          <div key={i} style={{ display: "flex", alignItems: "center", gap: 6, marginBottom: 4 }}>
            ...
            />
            {options.length > 1 && (
              <button onClick={() => onChange(options.filter((_, j) => j !== i))} style={{ background: "none", border: "none", cursor: "pointer", padding: 2, color: "rgba(0,0,0,0.25)", transition: "color 0.15s ease" }} onMouseEnter={(e) => (e.currentTarget.style.color = PD)} onMouseLeave={(e) => (e.currentTarget.style.color = "rgba(0,0,0,0.25)")}>
                <X size={11} strokeWidth={2} />
              </button>
            )}
          </div>
        ))}
        <button onClick={() => onChange([...options, ""])} style={{ marginTop: 4, display: "flex", alignItems: "center", gap: 4, fontSize: 10, fontWeight: 500, color: PD, background: "none", border: "none", cursor: "pointer", padding: 0 }}>
...
function PhotoConfig({ config, onChange }: { config: Record<string, unknown>; onChange: (c: Record<string, unknown>) => void }) {
  return <PhotoTagsConfig config={config} onChange={onChange} accentColor="#8b5cf6" />;
}
      ...
      <div style={{ marginTop: 6 }}>
        {items.map((item, i) => (
          <div key={i} style={{ display: "flex", alignItems: "center", gap: 6, marginBottom: 4 }}>
            ...
            {items.length > 1 && (
              <button onClick={() => onChange(items.filter((_, j) => j !== i))} style={{ background: "none", border: "none", cursor: "pointer", padding: 2, color: "rgba(0,0,0,0.25)" }} onMouseEnter={(e) => (e.currentTarget.style.color = PD)} onMouseLeave={(e) => (e.currentTarget.style.color = "rgba(0,0,0,0.25)")}>
                <X size={11} strokeWidth={2} />
              </button>
            )}
          </div>
        ))}
        <button onClick={() => onChange([...items, ""])} style={{ marginTop: 4, display: "flex", alignItems: "center", gap: 4, fontSize: 10, fontWeight: 500, color: PD, background: "none", border: "none", cursor: "pointer", padding: 0 }}>
          <Plus size={10} strokeWidth={2} />
          Hinzufügen
        </button>
      </div>
          ...
          {MATRIX_SUBTYPES.map((opt) => (
            <button
              ...
              style={{
                ...
                color: subtype === opt.value ? PD : "rgba(0,0,0,0.35)",
    ...
    <div style={{ marginTop: 10 }}>
      <div style={{ marginBottom: 14 }}>
        ...
        <div>
            ...
            return (
              <div key={i} style={{ display: "flex", alignItems: "center", gap: 6, marginBottom: 5 }}>
                <button
                  ...
                  style={{ width: 18, height: 18, borderRadius: 5, border: "none", cursor: ans.length > 0 ? "pointer" : "default", flexShrink: 0, display: "flex", alignItems: "center", justifyContent: "center", transition: "all 0.15s ease", ...(isChecked ? { background: `linear-gradient(180deg, ${P}, ${PD})`, boxShadow: `0 1px 3px ${P_BG_HVR}` } : { backgroundColor: "rgba(0,0,0,0.04)" }) }}
                >
                ...
                />
                {answers.length > 2 && (
                  <button onClick={() => { const next = answers.filter((_, j) => j !== i); const updatedBranches = branches.filter((b) => b.answer !== ans); onChange({ ...config, answers: next, branches: updatedBranches }); }} style={{ background: "none", border: "none", cursor: "pointer", padding: 2, color: "rgba(0,0,0,0.25)", transition: "color 0.15s ease" }} onMouseEnter={(e) => (e.currentTarget.style.color = PD)} onMouseLeave={(e) => (e.currentTarget.style.color = "rgba(0,0,0,0.25)")}>
                    <X size={11} strokeWidth={2} />
                  </button>
                )}
              </div>
          ...
          <button onClick={() => onChange({ ...config, answers: [...answers, ""] })} style={{ marginTop: 4, display: "flex", alignItems: "center", gap: 4, fontSize: 10, fontWeight: 500, color: PD, background: "none", border: "none", cursor: "pointer", padding: 0 }}>
            <Plus size={10} strokeWidth={2} />
            Antwort hinzufügen
          </button>
        </div>
        <div style={{ marginTop: 8, fontSize: 9, color: "rgba(0,0,0,0.3)", display: "flex", alignItems: "center", gap: 4 }}>
          <div style={{ width: 10, height: 10, borderRadius: 3, background: `linear-gradient(180deg, ${P}, ${PD})`, display: "inline-flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}>
      ...
      {orderedBranches.map(({ ans, branch }, idx) => (
        <div key={ans} style={{ marginBottom: 12 }}>
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 6, marginBottom: 8 }}>
            <div style={{ width: 16, height: 16, borderRadius: 4, flexShrink: 0, background: `linear-gradient(180deg, ${P}, ${PD})`, boxShadow: `0 1px 3px ${P_BG_HVR}`, display: "flex", alignItems: "center", justifyContent: "center" }}>
...
function TypeConfig({ question, onUpdate }: { question: Question; onUpdate: (q: Question) => void }) {
  ...
  switch (question.type) {
    case "single":
    case "multiple": return <ChoiceConfig options={(question.config.options as string[]) || [""]} onChange={(o) => setConfig({ ...question.config, options: o })} />;
    case "yesnomulti": return <YesNoMultiConfig config={question.config} onChange={setConfig} />;
    case "likert": return <LikertConfig config={question.config} onChange={setConfig} />;
    case "numeric": return <NumericConfig config={question.config} onChange={setConfig} />;
    case "slider": return <SliderConfig config={question.config} onChange={setConfig} />;
    case "photo": return <PhotoConfig config={question.config} onChange={setConfig} />;
    case "matrix": return <MatrixConfig config={question.config} onChange={setConfig} />;
    default: return null;
  }
}
    ...
    <>
      <div ref={pillRef} style={{ position: "relative", display: "inline-flex", alignItems: "center" }} onMouseEnter={() => { if (pillRef.current) setAnchorRect(pillRef.current.getBoundingClientRect()); setHovered(true); }} onMouseLeave={() => setHovered(false)}>
        <div onClick={(e) => { e.stopPropagation(); setLightbox(true); }} style={{ display: "flex", alignItems: "center", gap: 5, padding: "3px 8px 3px 6px", borderRadius: 5, backgroundColor: "rgba(0,0,0,0.04)", border: "1px solid rgba(0,0,0,0.07)", cursor: "pointer" }}>
          ...
          <button onClick={(e) => { e.stopPropagation(); onRemove(); }} style={{ display: "flex", background: "none", border: "none", cursor: "pointer", padding: 0, color: "rgba(0,0,0,0.25)", lineHeight: 1, transition: "color 0.12s ease" }} onMouseEnter={(e) => (e.currentTarget.style.color = PD)} onMouseLeave={(e) => (e.currentTarget.style.color = "rgba(0,0,0,0.25)")}>
...
function ImageAttachment({ value, onChange }: { value: string[]; onChange: (v: string[]) => void }) {
  ...
  return (
    <div style={{ marginTop: 8, display: "flex", alignItems: "center", flexWrap: "wrap", gap: 5 }}>
      ...
      {value.map((src, i) => <ImagePill key={i} src={src} index={i} onRemove={() => onChange(value.filter((_, j) => j !== i))} />)}
      {value.length === 0 ? (
    ...
    <div style={{ position: "relative" }}>
      ...
      >
        <div onClick={onToggle} style={{ display: "flex", alignItems: "center", gap: 8, padding: "0 14px", height: 44, cursor: "pointer", userSelect: "none" }}>
          <div draggable onDragStart={(e) => { e.stopPropagation(); e.dataTransfer.effectAllowed = "move"; const card = e.currentTarget.closest("[data-question-card]") as HTMLElement; if (card) { const clone = card.cloneNode(true) as HTMLElement; clone.style.width = `${card.offsetWidth}px`; clone.style.position = "absolute"; clone.style.top = "-9999px"; clone.style.left = "-9999px"; document.body.appendChild(clone); e.dataTransfer.setDragImage(clone, 20, 22); requestAnimationFrame(() => document.body.removeChild(clone)); } onDragStart(index); }} style={{ cursor: "grab", color: "rgba(0,0,0,0.15)", flexShrink: 0, display: "flex" }} onMouseDown={(e) => e.stopPropagation()}>
            <GripVertical size={14} strokeWidth={1.5} />
          </div>
          <div style={{ width: 18, height: 18, borderRadius: "50%", background: `linear-gradient(to bottom, ${P}, ${PD})`, display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}>
          ...
          {question.rules.length > 0 && <div style={{ width: 6, height: 6, borderRadius: "50%", backgroundColor: PD, flexShrink: 0 }} />}
        ...
        <div style={{ maxHeight: isExpanded ? 2000 : 0, opacity: isExpanded ? 1 : 0, overflow: "hidden", transition: "max-height 0.25s cubic-bezier(0.4,0,0.2,1), opacity 0.2s ease" }}>
          <div style={{ padding: "0 14px 14px" }}>
            ...
            <ImageAttachment value={(question.config.images as string[]) ?? []} onChange={(v) => onUpdate({ ...question, config: { ...question.config, images: v } })} />
            ...
            <div style={{ marginTop: 14 }}>
              <button onClick={(e) => { e.stopPropagation(); setLogicOpen(!logicOpen); }} style={{ display: "flex", alignItems: "center", gap: 7, width: "100%", padding: "8px 0 6px", fontSize: 11, fontWeight: 600, color: question.rules.length > 0 ? PD : "rgba(0,0,0,0.35)", background: "none", border: "none", cursor: "pointer", borderTop: "1px solid rgba(0,0,0,0.04)" }}>
                <Zap size={12} strokeWidth={2} style={{ flexShrink: 0 }} />
                <span style={{ flex: 1, textAlign: "left" }}>Bedingte Logik</span>
                {question.rules.length > 0 && <span style={{ fontSize: 9, fontWeight: 700, padding: "1px 7px", borderRadius: 10, backgroundColor: P_BG, color: PD }}>{question.rules.length} {question.rules.length === 1 ? "Regel" : "Regeln"}</span>}
            ...
            <div style={{ marginTop: 8, display: "flex", justifyContent: "flex-end" }}>
              <button onClick={(e) => { e.stopPropagation(); onDelete(); }} style={{ display: "flex", alignItems: "center", gap: 4, fontSize: 10, fontWeight: 500, color: PD, background: "none", border: "none", cursor: "pointer", padding: "4px 0", opacity: 0.7, transition: "opacity 0.15s ease" }} onMouseEnter={(e) => (e.currentTarget.style.opacity = "1")} onMouseLeave={(e) => (e.currentTarget.style.opacity = "0.7")}>
...
interface MhdModuleEditorProps {
  onClose: () => void;
...
export function MhdModuleEditor({ onClose, onSave, existingModule, availableChains = [], existingQuestions = [] }: MhdModuleEditorProps) {
  ...
  return (
    <div style={{ position: "fixed", inset: 0, zIndex: 200, backgroundColor: "#ffffff", display: "flex", flexDirection: "column", animation: "mhdModuleEditorIn 0.3s cubic-bezier(0.4,0,0.2,1) both" }}>
      <style>{`
      ...
      <div style={{ height: 56, borderBottom: "1px solid rgba(0,0,0,0.06)", display: "flex", alignItems: "center", justifyContent: "space-between", padding: "0 24px", flexShrink: 0 }}>
        ...
        <button
          onClick={async () => {
            if (isSaving) return;
            const mod: Module = {
              id: existingModule?.id ?? `mmod-${Date.now()}`,
          ...
          disabled={isSaving}
          style={{ display: "flex", alignItems: "center", gap: 5, padding: "7px 18px", fontSize: 11, fontWeight: 600, color: "#ffffff", background: `linear-gradient(to bottom, ${P}, ${PD})`, border: "none", borderRadius: 7, cursor: isSaving ? "not-allowed" : "pointer", opacity: isSaving ? 0.8 : 1, transition: "all 0.15s ease", letterSpacing: "0.01em", boxShadow: `inset 0 1px 0.6px rgba(255,255,255,0.33), inset 0 -1px 0 rgba(255,255,255,0.15), 0 0 0 1px ${PR}, 0 1px 6px rgba(124,58,237,0.25)` }}
        >
          ...
          <div ref={listRef} style={{ flex: 1, overflowY: "auto", padding: "16px 28px 28px", scrollbarWidth: "none", msOverflowStyle: "none" }} onDragOver={(e) => e.preventDefault()}>
            {questions.length === 0 && (
              <div style={{ display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", paddingTop: 80, gap: 10 }}>
                <div style={{ width: 44, height: 44, borderRadius: 11, backgroundColor: P_BG, display: "flex", alignItems: "center", justifyContent: "center" }}>
                  <Plus size={20} strokeWidth={1.4} color={P} />
                </div>
