...
const GD = "#65a30d";
const GR = "#4d7c0f";
const G_BG = "rgba(132,204,22,0.07)";
const G_BG_FAINT = "rgba(132,204,22,0.04)";

let _fxfid = 0;
function nextId(): string {
  _fxfid += 1;
  return `fxfb-${_fxfid}-${Date.now()}`;
}
                ...
                <button
                  ...
                  style={{
                    ...
                    boxShadow: isSelected
                      ? `inset 0 1px 0.6px rgba(255,255,255,0.33), 0 0 0 1px ${GR}, 0 1px 4px rgba(132,204,22,0.25)`
                      : "none",
...
function FlexFilterDropdown(props: FlexFilterDropdownProps) {
  const ref = useRef<HTMLDivElement>(null);
  useEffect(() => {
...
function FlexQuestionConfigSummary({ question }: { question: import("@/types/fragebogen").Question }) {
...
// ── Question Mini List ─────────────────────────────────────────
function FlexQuestionMiniList({ module }: { module: Module }) {
          ...
          <div key={q.id} style={{ borderRadius: 7, backgroundColor: isExpanded ? "rgba(0,0,0,0.025)" : "rgba(0,0,0,0.018)", overflow: "hidden" }}>
            ...
            <div style={{ maxHeight: isExpanded ? 200 : 0, overflow: "hidden", transition: "max-height 0.25s cubic-bezier(0.4,0,0.2,1)" }}>
              <div style={{ padding: "0 10px 8px" }}>
                <FlexQuestionConfigSummary question={q} />
              </div>
            </div>
          </div>
        );
...
// ── Module Row ─────────────────────────────────────────────────
function FlexModuleRow({ module, index, onRemove, onDragStart, onDragOver, onDrop, isDropTarget, expanded, onToggle }: {
      ...
      <div onDragOver={(e) => { e.preventDefault(); onDragOver(index); }} onDrop={(e) => { e.preventDefault(); onDrop(); }} style={{ borderRadius: expanded ? "8px 8px 0 0" : 8, border: "1px solid rgba(0,0,0,0.05)", borderBottom: expanded ? "1px solid rgba(0,0,0,0.04)" : "1px solid rgba(0,0,0,0.05)", backgroundColor: "#fafafa", overflow: "hidden" }}>
        ...
        {expanded && (
          <div style={{ borderTop: "1px solid rgba(0,0,0,0.04)", padding: "0 12px 8px", backgroundColor: "#f8f8f8" }}>
            <FlexQuestionMiniList module={module} />
          </div>
        )}
      </div>
    </div>
...
function FlexModulePreviewModal({ module, onClose }: { module: Module; onClose: () => void }) {
  useEffect(() => {
...
// ── Context Menu ───────────────────────────────────────────────
function FlexContextMenu({ x, y, onDetails, onClose }: { x: number; y: number; onDetails: () => void; onClose: () => void }) {
  const ref = useRef<HTMLDivElement>(null);
  useEffect(() => {
...
// ── Main Component ─────────────────────────────────────────────
export function FlexFragebogenEditor({
  onClose,
  onSave,
  existingFragebogen,
  availableModules,
  ...
  const btnActive: React.CSSProperties = { background: `linear-gradient(to bottom, ${G}, ${GD})`, color: "#fff", boxShadow: `inset 0 1px 0.6px rgba(255,255,255,0.33), inset 0 -1px 0 rgba(255,255,255,0.15), 0 0 0 1px ${GR}, 0 1px 6px rgba(132,204,22,0.25)` };
  ...
  return (
    <>
      <div style={{ position: "fixed", inset: 0, zIndex: 200, backgroundColor: "#ffffff", display: "flex", flexDirection: "column" }}>
        ...
        <div style={{ height: 56, borderBottom: "1px solid rgba(0,0,0,0.07)", display: "flex", alignItems: "center", gap: 12, padding: "0 20px", flexShrink: 0, backgroundColor: "#ffffff", boxShadow: "0 1px 4px rgba(0,0,0,0.04)" }}>
          ...
          <span style={{ fontSize: 14, fontWeight: 600, color: "#1a1a1a", letterSpacing: "-0.01em", flex: 1 }}>
            {existingFragebogen ? "Flexbesuch-Fragebogen bearbeiten" : "Neuer Flexbesuch-Fragebogen"}
          </span>
                ...
                {filteredLibraryModules.map((mod) => {
                  ...
                  return (
                    <div
                      ...
                      onMouseEnter={(e) => { if (!alreadyAdded) (e.currentTarget as HTMLElement).style.borderColor = `rgba(132,204,22,0.3)`; }}
          ...
          <div style={{ flex: 1, overflowY: "auto", backgroundColor: "#f5f5f7", padding: "24px 32px" }}>
            ...
            <div style={sectionStyle}>
              ...
              <div style={{ marginBottom: 16 }}>
                ...
                <input type="text" value={name} onChange={(e) => setName(e.target.value)} placeholder="z.B. Wöchentlicher Flexbesuch" style={inputStyle} />
              </div>
              <div>
              ...
              <div style={{ display: "flex", gap: 24 }}>
                ...
                <div style={{ flex: 1 }}>
                  ...
                  <div style={{ display: "flex", alignItems: "center", gap: 6, marginBottom: 8, flexWrap: "wrap" }}>
                    <div style={{ position: "relative" }}>
                      <button onClick={() => setOpenFilter(openFilter === "chain" ? null : "chain")} style={{ display: "flex", alignItems: "center", gap: 5, padding: "5px 10px", borderRadius: 7, fontSize: 10, fontWeight: 500, border: filterChain.length > 0 ? `1px solid rgba(132,204,22,0.25)` : "1px solid rgba(0,0,0,0.08)", backgroundColor: filterChain.length > 0 ? G_BG_FAINT : "#fff", color: filterChain.length > 0 ? GD : "rgba(0,0,0,0.55)", cursor: "pointer", fontFamily: "inherit" }}>
                        Handelskette
                      ...
                      {openFilter === "chain" && (
                        <FlexFilterDropdown mode="multi" options={ALL_CHAINS.map((c) => { const clr = chainColor(c); return { value: c, label: c, badge: { bg: clr.bg, text: clr.text } }; })} selected={filterChain} onToggle={(v) => setFilterChain((prev) => prev.includes(v) ? prev.filter((x) => x !== v) : [...prev, v])} onClear={() => setFilterChain([])} onClose={() => setOpenFilter(null)} />
                      )}
                    </div>

                    <div style={{ position: "relative" }}>
                      <button onClick={() => setOpenFilter(openFilter === "plz" ? null : "plz")} style={{ display: "flex", alignItems: "center", gap: 5, padding: "5px 10px", borderRadius: 7, fontSize: 10, fontWeight: 500, border: filterPlz.length > 0 ? `1px solid rgba(132,204,22,0.25)` : "1px solid rgba(0,0,0,0.08)", backgroundColor: filterPlz.length > 0 ? G_BG_FAINT : "#fff", color: filterPlz.length > 0 ? GD : "rgba(0,0,0,0.55)", cursor: "pointer", fontFamily: "inherit" }}>
                        PLZ
                      ...
                      {openFilter === "plz" && (
                        <FlexFilterDropdown mode="multi" options={ALL_PLZS.map((p) => ({ value: p, label: `${p} Wien` }))} selected={filterPlz} onToggle={(v) => setFilterPlz((prev) => prev.includes(v) ? prev.filter((x) => x !== v) : [...prev, v])} onClear={() => setFilterPlz([])} onClose={() => setOpenFilter(null)} />
                      )}
                    </div>

                    <div style={{ position: "relative" }}>
                      <button onClick={() => setOpenFilter(openFilter === "redmonat" ? null : "redmonat")} style={{ display: "flex", alignItems: "center", gap: 5, padding: "5px 10px", borderRadius: 7, fontSize: 10, fontWeight: 500, border: filterRedMonat ? `1px solid rgba(132,204,22,0.25)` : "1px solid rgba(0,0,0,0.08)", backgroundColor: filterRedMonat ? G_BG_FAINT : "#fff", color: filterRedMonat ? GD : "rgba(0,0,0,0.55)", cursor: "pointer", fontFamily: "inherit" }}>
                      ...
                      {openFilter === "redmonat" && (
                        <FlexFilterDropdown mode="single" options={RED_MONATS.map((rm) => ({ value: rm.label, label: rm.label, sub: `${fmtDate(rm.start)} – ${fmtDate(rm.end)}` }))} selected={filterRedMonat} onSelect={(v) => setFilterRedMonat(v)} onClose={() => setOpenFilter(null)} />
                      )}
                    </div>
                      ...
                      {(() => {
                        ...
                        return (
                          <button
                            ...
                            style={{ display: "flex", alignItems: "center", gap: 5, padding: "5px 12px", borderRadius: 7, fontSize: 10, fontWeight: 600, background: `linear-gradient(to bottom, ${G}, ${GD})`, color: "#fff", boxShadow: `inset 0 1px 0.6px rgba(255,255,255,0.33), inset 0 -1px 0 rgba(255,255,255,0.15), 0 0 0 1px ${GR}, 0 1px 6px rgba(132,204,22,0.25)`, border: "none", cursor: "pointer", fontFamily: "inherit" }}
                  ...
                  {scheduleType === "scheduled" && (
                    <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
                      <div>
                        ...
                        <FlexDatePicker value={startDate} onChange={setStartDate} placeholder="Startdatum wählen" />
                      </div>
                      <div>
                        <label style={{ ...labelStyle, display: "flex", alignItems: "center", gap: 5 }}><Calendar size={9} strokeWidth={2} />Bis</label>
                        <FlexDatePicker value={endDate} onChange={setEndDate} placeholder="Enddatum wählen" />
                      </div>
                      {startDate && endDate && (
      ...
      {contextMenu && (
        <FlexContextMenu x={contextMenu.x} y={contextMenu.y}
          onDetails={() => { const mod = availableModules.find((m) => m.id === contextMenu.moduleId); if (mod) setPreviewModule(mod); }}
          onClose={() => setContextMenu(null)}
        />
      )}

      {previewModule && <FlexModulePreviewModal module={previewModule} onClose={() => setPreviewModule(null)} />}
    </>
  );
}
