...
const YD = "#D97706";
const YR = "#B45309";
const Y_BG = "rgba(245,158,11,0.06)";
const Y_BG_HVR = "rgba(245,158,11,0.12)";
const Y_BORDER = "rgba(245,158,11,0.25)";
const Y_BORDER_HVR = "rgba(245,158,11,0.4)";
const Y_TEXT = "#92400e";

let _qid = 0;
function nextId(): string {
  _qid += 1;
  return `kq-${_qid}-${Date.now()}`;
}
        ...
        <div style={{ position: "absolute", top: "calc(100% + 4px)", left: 0, right: 0, zIndex: 50, backgroundColor: "#fff", borderRadius: 8, border: "1px solid rgba(0,0,0,0.06)", boxShadow: "0 4px 16px rgba(0,0,0,0.08), 0 1px 3px rgba(0,0,0,0.04)", padding: "4px", maxHeight: 180, overflowY: "auto" }}>
          {options.map((opt) => (
            <button
              ...
              style={{ width: "100%", display: "flex", alignItems: "center", justifyContent: "space-between", fontSize: 11, fontWeight: value === opt.value ? 600 : 400, color: value === opt.value ? YD : "#374151", padding: "6px 8px", borderRadius: 5, border: "none", cursor: "pointer", backgroundColor: value === opt.value ? Y_BG : "transparent", textAlign: "left", transition: "background-color 0.1s ease" }}
              onMouseEnter={(e) => { if (value !== opt.value) e.currentTarget.style.backgroundColor = "rgba(0,0,0,0.025)"; }}
              onMouseLeave={(e) => { e.currentTarget.style.backgroundColor = value === opt.value ? Y_BG : "transparent"; }}
            >
              <span style={{ overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap", flex: 1 }}>{opt.label}</span>
              {value === opt.value && <Check size={12} strokeWidth={2.5} color={YD} style={{ flexShrink: 0, marginLeft: 6 }} />}
            </button>
          ))}
        </div>
      )}
  ...
  return (
    <div style={{ paddingTop: 6 }}>
        ...
        return (
          <div key={rule.id} style={{ border: "1px solid rgba(0,0,0,0.05)", borderRadius: 10, padding: "12px 14px 14px", marginBottom: 8, backgroundColor: "#fff" }}>
            <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 12 }}>
              ...
              <button onClick={() => removeRule(rule.id)} style={{ background: "none", border: "none", cursor: "pointer", padding: 2, color: "rgba(0,0,0,0.2)", transition: "color 0.15s ease" }} onMouseEnter={(e) => (e.currentTarget.style.color = YD)} onMouseLeave={(e) => (e.currentTarget.style.color = "rgba(0,0,0,0.2)")}>
                  ...
                  {targetCandidates.map((tq) => {
                    ...
                    return (
                      <button key={tq.id} onClick={() => toggleTarget(rule.id, tq.id)}
                        style={{ display: "flex", alignItems: "center", gap: 8, fontSize: 11, fontWeight: 400, padding: "6px 8px", borderRadius: 6, border: "none", cursor: "pointer", transition: "background-color 0.12s ease", backgroundColor: isSelected ? Y_BG : "transparent", color: "#374151", textAlign: "left", width: "100%" }}
                        onMouseEnter={(e) => { if (!isSelected) e.currentTarget.style.backgroundColor = "rgba(0,0,0,0.02)"; }}
                        onMouseLeave={(e) => { e.currentTarget.style.backgroundColor = isSelected ? Y_BG : "transparent"; }}
                      >
                        <div style={{ width: 16, height: 16, borderRadius: 4, border: isSelected ? "none" : "1.5px solid rgba(0,0,0,0.12)", backgroundColor: isSelected ? YD : "transparent", display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0, transition: "all 0.12s ease" }}>
      ...
      <button
        ...
        style={{ display: "flex", alignItems: "center", justifyContent: "center", gap: 5, width: "100%", fontSize: 10, fontWeight: 600, color: YD, backgroundColor: Y_BG, border: `1px dashed ${Y_BORDER}`, borderRadius: 8, cursor: "pointer", padding: "8px 0", transition: "all 0.15s ease" }}
        onMouseEnter={(e) => { e.currentTarget.style.backgroundColor = Y_BG_HVR; e.currentTarget.style.borderColor = Y_BORDER_HVR; }}
        onMouseLeave={(e) => { e.currentTarget.style.backgroundColor = Y_BG; e.currentTarget.style.borderColor = Y_BORDER; }}
      >
...
function Toggle({ value, onChange }: { value: boolean; onChange: (v: boolean) => void }) {
  return (
    <button
      ...
      style={{ width: 32, height: 18, borderRadius: 9, backgroundColor: value ? YD : "rgba(0,0,0,0.12)", border: "none", cursor: "pointer", position: "relative", transition: "background-color 0.2s ease", flexShrink: 0 }}
    >
      ...
      <div style={{ marginTop: 6 }}>
        {options.map((opt, i) => (
          <div key={i} style={{ display: "flex", alignItems: "center", gap: 6, marginBottom: 4 }}>
            ...
            />
            {options.length > 1 && (
              <button onClick={() => onChange(options.filter((_, j) => j !== i))} style={{ background: "none", border: "none", cursor: "pointer", padding: 2, color: "rgba(0,0,0,0.25)", transition: "color 0.15s ease" }} onMouseEnter={(e) => (e.currentTarget.style.color = YD)} onMouseLeave={(e) => (e.currentTarget.style.color = "rgba(0,0,0,0.25)")}>
                <X size={11} strokeWidth={2} />
              </button>
            )}
          </div>
        ))}
        <button onClick={() => onChange([...options, ""])} style={{ marginTop: 4, display: "flex", alignItems: "center", gap: 4, fontSize: 10, fontWeight: 500, color: YD, background: "none", border: "none", cursor: "pointer", padding: 0 }}>
      ...
      <div style={{ marginTop: 6 }}>
        {items.map((item, i) => (
          <div key={i} style={{ display: "flex", alignItems: "center", gap: 6, marginBottom: 4 }}>
            ...
            {items.length > 1 && (
              <button onClick={() => onChange(items.filter((_, j) => j !== i))} style={{ background: "none", border: "none", cursor: "pointer", padding: 2, color: "rgba(0,0,0,0.25)" }} onMouseEnter={(e) => (e.currentTarget.style.color = YD)} onMouseLeave={(e) => (e.currentTarget.style.color = "rgba(0,0,0,0.25)")}>
                <X size={11} strokeWidth={2} />
              </button>
            )}
          </div>
        ))}
        <button onClick={() => onChange([...items, ""])} style={{ marginTop: 4, display: "flex", alignItems: "center", gap: 4, fontSize: 10, fontWeight: 500, color: YD, background: "none", border: "none", cursor: "pointer", padding: 0 }}>
          <Plus size={10} strokeWidth={2} />
          Hinzufügen
        </button>
      </div>
    ...
    <div style={{ marginTop: 10 }}>
      <div style={{ marginBottom: 14 }}>
        ...
        <div>
            ...
            return (
              <div key={i} style={{ display: "flex", alignItems: "center", gap: 6, marginBottom: 5 }}>
                <button
                  ...
                  style={{ width: 18, height: 18, borderRadius: 5, border: "none", cursor: ans.length > 0 ? "pointer" : "default", flexShrink: 0, display: "flex", alignItems: "center", justifyContent: "center", transition: "all 0.15s ease", ...(isChecked ? { background: `linear-gradient(180deg, ${Y}, ${YD})`, boxShadow: `0 1px 3px ${Y_BG_HVR}` } : { backgroundColor: "rgba(0,0,0,0.04)" }) }}
                >
                ...
                />
                {answers.length > 2 && (
                  <button onClick={() => { const next = answers.filter((_, j) => j !== i); const updatedBranches = branches.filter((b) => b.answer !== ans); onChange({ ...config, answers: next, branches: updatedBranches }); }} style={{ background: "none", border: "none", cursor: "pointer", padding: 2, color: "rgba(0,0,0,0.25)", transition: "color 0.15s ease" }} onMouseEnter={(e) => (e.currentTarget.style.color = YD)} onMouseLeave={(e) => (e.currentTarget.style.color = "rgba(0,0,0,0.25)")}>
                    <X size={11} strokeWidth={2} />
                  </button>
                )}
              </div>
          ...
          <button onClick={() => onChange({ ...config, answers: [...answers, ""] })} style={{ marginTop: 4, display: "flex", alignItems: "center", gap: 4, fontSize: 10, fontWeight: 500, color: YD, background: "none", border: "none", cursor: "pointer", padding: 0 }}>
            <Plus size={10} strokeWidth={2} />
            Antwort hinzufügen
          </button>
        </div>
        <div style={{ marginTop: 8, fontSize: 9, color: "rgba(0,0,0,0.3)", display: "flex", alignItems: "center", gap: 4 }}>
          <div style={{ width: 10, height: 10, borderRadius: 3, background: `linear-gradient(180deg, ${Y}, ${YD})`, display: "inline-flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}>
      ...
      {orderedBranches.map(({ ans, branch }, idx) => (
        <div key={ans} style={{ marginBottom: 12 }}>
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 6, marginBottom: 8 }}>
            <div style={{ width: 16, height: 16, borderRadius: 4, flexShrink: 0, background: `linear-gradient(180deg, ${Y}, ${YD})`, boxShadow: `0 1px 3px ${Y_BG_HVR}`, display: "flex", alignItems: "center", justifyContent: "center" }}>
    ...
    <>
      <div ref={pillRef} style={{ position: "relative", display: "inline-flex", alignItems: "center" }} onMouseEnter={() => { if (pillRef.current) setAnchorRect(pillRef.current.getBoundingClientRect()); setHovered(true); }} onMouseLeave={() => setHovered(false)}>
        <div onClick={(e) => { e.stopPropagation(); setLightbox(true); }} style={{ display: "flex", alignItems: "center", gap: 5, padding: "3px 8px 3px 6px", borderRadius: 5, backgroundColor: "rgba(0,0,0,0.04)", border: "1px solid rgba(0,0,0,0.07)", cursor: "pointer" }}>
          ...
          <button onClick={(e) => { e.stopPropagation(); onRemove(); }} style={{ display: "flex", background: "none", border: "none", cursor: "pointer", padding: 0, color: "rgba(0,0,0,0.25)", lineHeight: 1, transition: "color 0.12s ease" }} onMouseEnter={(e) => (e.currentTarget.style.color = YD)} onMouseLeave={(e) => (e.currentTarget.style.color = "rgba(0,0,0,0.25)")}>
    ...
    <div style={{ position: "relative" }}>
      ...
      >
        ...
        <div onClick={onToggle} style={{ display: "flex", alignItems: "center", gap: 8, padding: "0 14px", height: 44, cursor: "pointer", userSelect: "none" }}>
          ...
          {question.rules.length > 0 && <div style={{ width: 6, height: 6, borderRadius: "50%", backgroundColor: YD, flexShrink: 0 }} />}
          ...
          <div style={{ padding: "0 14px 14px" }}>
            ...
            <div style={{ marginTop: 14 }}>
              <button onClick={(e) => { e.stopPropagation(); setLogicOpen(!logicOpen); }} style={{ display: "flex", alignItems: "center", gap: 7, width: "100%", padding: "8px 0 6px", fontSize: 11, fontWeight: 600, color: question.rules.length > 0 ? YD : "rgba(0,0,0,0.35)", background: "none", border: "none", cursor: "pointer", borderTop: "1px solid rgba(0,0,0,0.04)" }}>
                ...
                {question.rules.length > 0 && <span style={{ fontSize: 9, fontWeight: 700, padding: "1px 7px", borderRadius: 10, backgroundColor: Y_BG, color: YD }}>{question.rules.length} {question.rules.length === 1 ? "Regel" : "Regeln"}</span>}
            ...
            <div style={{ marginTop: 8, display: "flex", justifyContent: "flex-end" }}>
              <button onClick={(e) => { e.stopPropagation(); onDelete(); }} style={{ display: "flex", alignItems: "center", gap: 4, fontSize: 10, fontWeight: 500, color: YD, background: "none", border: "none", cursor: "pointer", padding: "4px 0", opacity: 0.7, transition: "opacity 0.15s ease" }} onMouseEnter={(e) => (e.currentTarget.style.opacity = "1")} onMouseLeave={(e) => (e.currentTarget.style.opacity = "0.7")}>
...
export function KuehlerModuleEditor({ onClose, onSave, existingModule }: KuehlerModuleEditorProps) {
  ...
  return (
    <div style={{ position: "fixed", inset: 0, zIndex: 200, backgroundColor: "#ffffff", display: "flex", flexDirection: "column", animation: "kuehlerModuleEditorIn 0.3s cubic-bezier(0.4,0,0.2,1) both" }}>
      <style>{`
        @keyframes kuehlerModuleEditorIn { from { opacity: 0; transform: scale(0.98); } to { opacity: 1; transform: scale(1); } }
      ...
      <div style={{ flex: 1, display: "flex", minHeight: 0 }}>
        ...
        <div style={{ width: 220, backgroundColor: "rgba(0,0,0,0.02)", borderRight: "1px solid rgba(0,0,0,0.06)", padding: "20px 12px", flexShrink: 0, overflowY: "auto" }}>
          ...
          <button style={{ display: "flex", alignItems: "center", gap: 6, padding: "4px 6px", fontSize: 11, fontWeight: 500, color: YD, background: "none", border: "none", cursor: "pointer" }}>
          ...
          <div ref={listRef} style={{ flex: 1, overflowY: "auto", padding: "16px 28px 28px", scrollbarWidth: "none", msOverflowStyle: "none" }} onDragOver={(e) => e.preventDefault()}>
            {questions.length === 0 && (
              <div style={{ display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", paddingTop: 80, gap: 10 }}>
                <div style={{ width: 44, height: 44, borderRadius: 11, backgroundColor: Y_BG, display: "flex", alignItems: "center", justifyContent: "center" }}>
                  <Plus size={20} strokeWidth={1.4} color={Y} />
                </div>
