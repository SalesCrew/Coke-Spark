...
const GD = "#0e7490";
const GR = "#155e75";
const G_BG = "rgba(8,145,178,0.07)";
const G_BG_FAINT = "rgba(8,145,178,0.04)";

let _bfid = 0;
function nextId(): string {
  _bfid += 1;
  return `bfb-${_bfid}-${Date.now()}`;
}
      ...
      {open && (
        <div style={{
                ...
                <button
                  ...
                  style={{
                    ...
                    background: isSelected
                      ? `linear-gradient(to bottom, ${G}, ${GD})`
                      : isToday ? G_BG : "transparent",
                    color: isSelected ? "#fff" : isToday ? GD : "#111",
                    fontSize: 11, fontWeight: isSelected || isToday ? 600 : 400,
                    cursor: "pointer", fontFamily: "inherit",
                    boxShadow: isSelected
                      ? `inset 0 1px 0.6px rgba(255,255,255,0.33), 0 0 0 1px ${GR}, 0 1px 4px rgba(8,145,178,0.25)`
                      : "none",
                    transition: "background 0.12s",
                  }}
                  onMouseEnter={(e) => { if (!isSelected) e.currentTarget.style.background = "rgba(0,0,0,0.04)"; }}
                  onMouseLeave={(e) => { if (!isSelected) e.currentTarget.style.background = isToday ? G_BG : "transparent"; }}
                >
                  {d}
                </button>
              );
          ...
          <div style={{ display: "flex", justifyContent: "space-between", marginTop: 10, paddingTop: 8, borderTop: "1px solid rgba(0,0,0,0.05)" }}>
            ...
            <button type="button" onClick={goToday} style={{ background: "none", border: "none", cursor: "pointer", fontSize: 10, color: GD, fontFamily: "inherit", fontWeight: 600, padding: "2px 4px" }}>Heute</button>
          </div>
        </div>
      )}
    </div>
...
function BillaFilterDropdown(props: BillaFilterDropdownProps) {
  const ref = useRef<HTMLDivElement>(null);
  useEffect(() => {
      ...
      {props.mode === "multi" ? (
        <>
          {props.options.map((opt) => {
            ...
            return (
              <div key={opt.value} onClick={() => props.onToggle(opt.value)} style={{ display: "flex", alignItems: "center", gap: 9, padding: "7px 14px", cursor: "pointer", transition: "background-color 0.1s ease" }} onMouseEnter={(e) => (e.currentTarget.style.backgroundColor = "rgba(0,0,0,0.025)")} onMouseLeave={(e) => (e.currentTarget.style.backgroundColor = "transparent")}>
                <div style={{ width: 14, height: 14, borderRadius: 4, flexShrink: 0, border: checked ? "none" : "1.5px solid rgba(0,0,0,0.15)", backgroundColor: checked ? GD : "transparent", display: "flex", alignItems: "center", justifyContent: "center", transition: "all 0.15s ease" }}>
          ...
          {props.selected.length > 0 && (
            <div style={{ borderTop: "1px solid rgba(0,0,0,0.05)", padding: "6px 14px 2px", display: "flex", justifyContent: "flex-end" }}>
              <button onClick={props.onClear} style={{ background: "none", border: "none", cursor: "pointer", fontSize: 10, color: "rgba(0,0,0,0.35)", fontWeight: 500, padding: "2px 0", fontFamily: "inherit" }} onMouseEnter={(e) => (e.currentTarget.style.color = GD)} onMouseLeave={(e) => (e.currentTarget.style.color = "rgba(0,0,0,0.35)")}>Zurücksetzen</button>
            </div>
          )}
        </>
      ) : (
        <>
          {props.options.map((opt) => {
            ...
            return (
              <div key={opt.value} onClick={() => { props.onSelect(active ? null : opt.value); props.onClose(); }} style={{ display: "flex", alignItems: "center", gap: 9, padding: "7px 14px", cursor: "pointer", transition: "background-color 0.1s ease", backgroundColor: active ? G_BG_FAINT : "transparent" }} onMouseEnter={(e) => { if (!active) e.currentTarget.style.backgroundColor = "rgba(0,0,0,0.025)"; }} onMouseLeave={(e) => { e.currentTarget.style.backgroundColor = active ? G_BG_FAINT : "transparent"; }}>
                <div style={{ width: 14, height: 14, borderRadius: "50%", flexShrink: 0, border: active ? "none" : "1.5px solid rgba(0,0,0,0.15)", backgroundColor: active ? GD : "transparent", display: "flex", alignItems: "center", justifyContent: "center", transition: "all 0.15s ease" }}>
                  {active && <Check size={7} strokeWidth={3} color="#fff" />}
                </div>
                <div>
                  <div style={{ fontSize: 11, color: active ? GD : "#374151", fontWeight: active ? 600 : 500 }}>{opt.label}</div>
          ...
          {props.selected && (
            <div style={{ borderTop: "1px solid rgba(0,0,0,0.05)", padding: "6px 14px 2px", display: "flex", justifyContent: "flex-end" }}>
              <button onClick={() => { props.onSelect(null); props.onClose(); }} style={{ background: "none", border: "none", cursor: "pointer", fontSize: 10, color: "rgba(0,0,0,0.35)", fontWeight: 500, padding: "2px 0", fontFamily: "inherit" }} onMouseEnter={(e) => (e.currentTarget.style.color = GD)} onMouseLeave={(e) => (e.currentTarget.style.color = "rgba(0,0,0,0.35)")}>Zurücksetzen</button>
            </div>
          )}
        </>
      )}
...
function BillaQuestionConfigSummary({ question }: { question: import("@/types/fragebogen").Question }) {
...
// ── Question Mini List ─────────────────────────────────────────
function BillaQuestionMiniList({ module }: { module: Module }) {
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 3, padding: "6px 0 4px" }}>
      {module.questions.map((q, idx) => {
        ...
        return (
          <div key={q.id} style={{ borderRadius: 7, backgroundColor: isExpanded ? "rgba(0,0,0,0.025)" : "rgba(0,0,0,0.018)", overflow: "hidden" }}>
            ...
            <div style={{ maxHeight: isExpanded ? 200 : 0, overflow: "hidden", transition: "max-height 0.25s cubic-bezier(0.4,0,0.2,1)" }}>
              <div style={{ padding: "0 10px 8px" }}>
                <BillaQuestionConfigSummary question={q} />
              </div>
            </div>
          </div>
        );
...
// ── Module Row ─────────────────────────────────────────────────
function BillaModuleRow({ module, index, onRemove, onDragStart, onDragOver, onDrop, isDropTarget, expanded, onToggle }: {
  ...
  return (
    <div style={{ position: "relative", marginBottom: 6 }}>
      {isDropTarget && <div style={{ position: "absolute", top: -2, left: 0, right: 0, height: 3, borderRadius: 2, backgroundColor: GD, opacity: 0.6 }} />}
      <div onDragOver={(e) => { e.preventDefault(); onDragOver(index); }} onDrop={(e) => { e.preventDefault(); onDrop(); }} style={{ borderRadius: expanded ? "8px 8px 0 0" : 8, border: "1px solid rgba(0,0,0,0.05)", borderBottom: expanded ? "1px solid rgba(0,0,0,0.04)" : "1px solid rgba(0,0,0,0.05)", backgroundColor: "#fafafa", overflow: "hidden" }}>
        <div style={{ display: "flex", alignItems: "center", gap: 8, padding: "9px 12px" }}>
          ...
          <button onClick={onToggle} style={{ background: "none", border: "none", cursor: "pointer", padding: 2, color: "rgba(0,0,0,0.25)", display: "flex", alignItems: "center" }} onMouseEnter={(e) => (e.currentTarget.style.color = "#374151")} onMouseLeave={(e) => (e.currentTarget.style.color = "rgba(0,0,0,0.25)")}>
            <ChevronDown size={13} strokeWidth={2} style={{ transition: "transform 0.2s ease", transform: expanded ? "rotate(180deg)" : "rotate(0deg)" }} />
          </button>
          <button onClick={onRemove} style={{ background: "none", border: "none", cursor: "pointer", padding: 2, color: "rgba(0,0,0,0.2)", display: "flex", alignItems: "center" }} onMouseEnter={(e) => (e.currentTarget.style.color = GD)} onMouseLeave={(e) => (e.currentTarget.style.color = "rgba(0,0,0,0.2)")}>
            <X size={12} strokeWidth={2} />
          </button>
        </div>
        {expanded && (
          <div style={{ borderTop: "1px solid rgba(0,0,0,0.04)", padding: "0 12px 8px", backgroundColor: "#f8f8f8" }}>
            <BillaQuestionMiniList module={module} />
          </div>
        )}
      </div>
    </div>
...
function BillaModulePreviewModal({ module, onClose }: { module: Module; onClose: () => void }) {
  useEffect(() => {
        ...
        <div style={{ padding: "18px 20px 14px", borderBottom: "1px solid rgba(0,0,0,0.06)", flexShrink: 0, display: "flex", alignItems: "flex-start", gap: 12 }}>
          <div style={{ flex: 1 }}>
            ...
            <div style={{ display: "flex", alignItems: "center", gap: 8, marginTop: 8 }}>
              <span style={{ fontSize: 9, fontWeight: 600, padding: "2px 8px", borderRadius: 4, backgroundColor: G_BG, color: GD }}>{module.questions.length} {module.questions.length === 1 ? "Frage" : "Fragen"}</span>
          ...
          <button onClick={onClose} style={{ background: "none", border: "none", cursor: "pointer", padding: 4, color: "rgba(0,0,0,0.35)", display: "flex", alignItems: "center", flexShrink: 0 }} onMouseEnter={(e) => (e.currentTarget.style.color = GD)} onMouseLeave={(e) => (e.currentTarget.style.color = "rgba(0,0,0,0.35)")}>
...
// ── Context Menu ───────────────────────────────────────────────
function BillaLibraryContextMenu({ x, y, onDetails, onClose }: { x: number; y: number; onDetails: () => void; onClose: () => void }) {
  const ref = useRef<HTMLDivElement>(null);
  useEffect(() => {
  ...
  return (
    <div ref={ref} style={{ position: "fixed", left: safeX, top: safeY, zIndex: 400, backgroundColor: "#ffffff", borderRadius: 8, boxShadow: "0 4px 20px rgba(0,0,0,0.10), 0 1px 4px rgba(0,0,0,0.06), 0 0 0 1px rgba(0,0,0,0.05)", padding: "4px", minWidth: menuW }}>
      <button onClick={() => { onDetails(); onClose(); }} style={{ display: "flex", alignItems: "center", gap: 8, width: "100%", padding: "7px 10px", background: "none", border: "none", cursor: "pointer", fontSize: 12, fontWeight: 500, color: "#374151", borderRadius: 5, textAlign: "left" }} onMouseEnter={(e) => (e.currentTarget.style.backgroundColor = "rgba(0,0,0,0.04)")} onMouseLeave={(e) => (e.currentTarget.style.backgroundColor = "transparent")}>
...
// ── Main Component ─────────────────────────────────────────────
export function BillaFragebogenEditor({
  onClose,
  onSave,
  existingFragebogen,
  availableModules,
  ...
  const btnActive: React.CSSProperties = { background: `linear-gradient(to bottom, ${G}, ${GD})`, color: "#fff", boxShadow: `inset 0 1px 0.6px rgba(255,255,255,0.33), inset 0 -1px 0 rgba(255,255,255,0.15), 0 0 0 1px ${GR}, 0 1px 6px rgba(8,145,178,0.25)` };
    ...
    <>
      <div style={{ position: "fixed", inset: 0, zIndex: 200, backgroundColor: "#ffffff", display: "flex", flexDirection: "column" }}>
        <div style={{ height: 56, borderBottom: "1px solid rgba(0,0,0,0.07)", display: "flex", alignItems: "center", gap: 12, padding: "0 20px", flexShrink: 0, backgroundColor: "#ffffff", boxShadow: "0 1px 4px rgba(0,0,0,0.04)" }}>
          <button onClick={onClose} style={{ background: "none", border: "none", cursor: "pointer", padding: 4, color: "rgba(0,0,0,0.4)", display: "flex", alignItems: "center" }} onMouseEnter={(e) => (e.currentTarget.style.color = GD)} onMouseLeave={(e) => (e.currentTarget.style.color = "rgba(0,0,0,0.4)")}>
          ...
          <span style={{ fontSize: 14, fontWeight: 600, color: "#1a1a1a", letterSpacing: "-0.01em", flex: 1 }}>
            {existingFragebogen ? "Billa-Fragebogen bearbeiten" : "Neuer Billa-Fragebogen"}
          </span>
                ...
                {filteredLibraryModules.map((mod) => {
                  ...
                  return (
                    <div
                      ...
                      style={{ padding: "8px 12px", borderRadius: 8, border: alreadyAdded ? `1px solid ${G_BG}` : "1px solid rgba(0,0,0,0.06)", backgroundColor: alreadyAdded ? G_BG_FAINT : "#ffffff", cursor: alreadyAdded ? "default" : "pointer", transition: "all 0.15s ease", display: "flex", alignItems: "center", gap: 8, userSelect: "none" }}
                      onMouseEnter={(e) => { if (!alreadyAdded) (e.currentTarget as HTMLElement).style.borderColor = `rgba(8,145,178,0.3)`; }}
                    ...
                    >
                      ...
                      {alreadyAdded && <Check size={10} strokeWidth={2.5} color={GD} />}
                    </div>
                  );
                })}
              </div>
          ...
          <div style={{ flex: 1, overflowY: "auto", backgroundColor: "#f5f5f7", padding: "24px 32px" }}>
            <div style={sectionStyle}>
              ...
              <div style={{ marginBottom: 16 }}>
                ...
                <input type="text" value={name} onChange={(e) => setName(e.target.value)} placeholder="z.B. Wöchentlicher Billa-Besuch" style={inputStyle} />
              </div>
              <div>
              ...
              <div style={{ marginTop: 14, display: "flex", alignItems: "center", justifyContent: "space-between" }}>
                ...
                <button onClick={() => setNurEinmal((v) => !v)} style={{ width: 38, height: 22, borderRadius: 99, border: "none", cursor: "pointer", flexShrink: 0, background: nurEinmal ? GD : "rgba(0,0,0,0.1)", transition: "all 0.18s ease", position: "relative" }}>
            ...
            <div style={sectionStyle}>
              <div style={sectionHeadingStyle}>
                Module
                <span style={{ marginLeft: 8, fontSize: 9, fontWeight: 600, padding: "2px 7px", borderRadius: 4, backgroundColor: G_BG, color: GD }}>{selectedModules.length}</span>
              </div>
              ...
              {selectedModules.length > 0 && (
                <button onClick={() => {}} style={{ marginTop: 8, display: "flex", alignItems: "center", gap: 5, fontSize: 10, fontWeight: 500, color: GD, background: "none", border: "none", cursor: "pointer", padding: 0 }}>
              ...
              <div style={{ display: "flex", gap: 24 }}>
                <div style={{ flex: 1 }}>
                  ...
                  <div style={{ display: "flex", alignItems: "center", gap: 6, marginBottom: 8, flexWrap: "wrap" }}>
                    <div style={{ position: "relative" }}>
                      <button onClick={() => setOpenFilter(openFilter === "chain" ? null : "chain")} style={{ display: "flex", alignItems: "center", gap: 5, padding: "5px 10px", borderRadius: 7, fontSize: 10, fontWeight: 500, border: filterChain.length > 0 ? `1px solid rgba(8,145,178,0.25)` : "1px solid rgba(0,0,0,0.08)", backgroundColor: filterChain.length > 0 ? G_BG_FAINT : "#fff", color: filterChain.length > 0 ? GD : "rgba(0,0,0,0.55)", cursor: "pointer", fontFamily: "inherit" }}>
                        Handelskette
                        {filterChain.length > 0 && <span style={{ fontSize: 8, fontWeight: 700, padding: "1px 5px", borderRadius: 10, backgroundColor: GD, color: "#fff", lineHeight: 1.4 }}>{filterChain.length}</span>}
                      ...
                      {openFilter === "chain" && (
                        <BillaFilterDropdown mode="multi" options={ALL_CHAINS.map((c) => { const clr = chainColor(c); return { value: c, label: c, badge: { bg: clr.bg, text: clr.text } }; })} selected={filterChain} onToggle={(v) => setFilterChain((prev) => prev.includes(v) ? prev.filter((x) => x !== v) : [...prev, v])} onClear={() => setFilterChain([])} onClose={() => setOpenFilter(null)} />
                      )}
                    </div>

                    <div style={{ position: "relative" }}>
                      <button onClick={() => setOpenFilter(openFilter === "plz" ? null : "plz")} style={{ display: "flex", alignItems: "center", gap: 5, padding: "5px 10px", borderRadius: 7, fontSize: 10, fontWeight: 500, border: filterPlz.length > 0 ? `1px solid rgba(8,145,178,0.25)` : "1px solid rgba(0,0,0,0.08)", backgroundColor: filterPlz.length > 0 ? G_BG_FAINT : "#fff", color: filterPlz.length > 0 ? GD : "rgba(0,0,0,0.55)", cursor: "pointer", fontFamily: "inherit" }}>
                        PLZ
                        {filterPlz.length > 0 && <span style={{ fontSize: 8, fontWeight: 700, padding: "1px 5px", borderRadius: 10, backgroundColor: GD, color: "#fff", lineHeight: 1.4 }}>{filterPlz.length}</span>}
                      ...
                      {openFilter === "plz" && (
                        <BillaFilterDropdown mode="multi" options={ALL_PLZS.map((p) => ({ value: p, label: `${p} Wien` }))} selected={filterPlz} onToggle={(v) => setFilterPlz((prev) => prev.includes(v) ? prev.filter((x) => x !== v) : [...prev, v])} onClear={() => setFilterPlz([])} onClose={() => setOpenFilter(null)} />
                      )}
                    </div>

                    <div style={{ position: "relative" }}>
                      <button onClick={() => setOpenFilter(openFilter === "redmonat" ? null : "redmonat")} style={{ display: "flex", alignItems: "center", gap: 5, padding: "5px 10px", borderRadius: 7, fontSize: 10, fontWeight: 500, border: filterRedMonat ? `1px solid rgba(8,145,178,0.25)` : "1px solid rgba(0,0,0,0.08)", backgroundColor: filterRedMonat ? G_BG_FAINT : "#fff", color: filterRedMonat ? GD : "rgba(0,0,0,0.55)", cursor: "pointer", fontFamily: "inherit" }}>
                      ...
                      {openFilter === "redmonat" && (
                        <BillaFilterDropdown mode="single" options={RED_MONATS.map((rm) => ({ value: rm.label, label: rm.label, sub: `${fmtDate(rm.start)} – ${fmtDate(rm.end)}` }))} selected={filterRedMonat} onSelect={(v) => setFilterRedMonat(v)} onClose={() => setOpenFilter(null)} />
                      )}
                    </div>
                    ...
                    <div style={{ marginLeft: "auto", display: "flex", alignItems: "stretch", gap: 5 }}>
                      {(() => {
                        ...
                        return (
                          <button
                            ...
                            style={{ display: "flex", alignItems: "center", gap: 5, padding: "5px 12px", borderRadius: 7, fontSize: 10, fontWeight: 600, background: `linear-gradient(to bottom, ${G}, ${GD})`, color: "#fff", boxShadow: `inset 0 1px 0.6px rgba(255,255,255,0.33), inset 0 -1px 0 rgba(255,255,255,0.15), 0 0 0 1px ${GR}, 0 1px 6px rgba(8,145,178,0.25)`, border: "none", cursor: "pointer", fontFamily: "inherit" }}
                      ...
                      {(filterChain.length > 0 || filterPlz.length > 0 || filterRedMonat) && (
                        <button onClick={() => { setFilterChain([]); setFilterPlz([]); setFilterRedMonat(null); setOpenFilter(null); }} title="Filter zurücksetzen" style={{ display: "flex", alignItems: "center", justifyContent: "center", alignSelf: "stretch", padding: "0 10px", borderRadius: 7, flexShrink: 0, background: "linear-gradient(to bottom, #1a1a1a, #111111)", boxShadow: "inset 0 1px 0.6px rgba(255,255,255,0.12), inset 0 -1px 0 rgba(255,255,255,0.06), 0 0 0 1px #000, 0 1px 6px rgba(0,0,0,0.22)", border: "none", cursor: "pointer", color: "#fff" }} onMouseEnter={(e) => (e.currentTarget.style.opacity = "0.75")} onMouseLeave={(e) => (e.currentTarget.style.opacity = "1")}>
                  ...
                  {(filterChain.length > 0 || filterPlz.length > 0 || filterRedMonat) && (
                    <div style={{ display: "flex", alignItems: "center", gap: 6, flexWrap: "wrap", marginBottom: 8, padding: "6px 10px", borderRadius: 8, backgroundColor: G_BG_FAINT }}>
                      ...
                      {filterChain.length > 0 && <button onClick={() => setFilterChain([])} style={{ display: "flex", alignItems: "center", gap: 4, padding: "2px 7px", borderRadius: 5, fontSize: 9, fontWeight: 600, backgroundColor: G_BG, color: GD, border: "none", cursor: "pointer", fontFamily: "inherit" }}>{filterChain.join(", ")}<X size={8} strokeWidth={2.5} /></button>}
                      {filterPlz.length > 0 && <button onClick={() => setFilterPlz([])} style={{ display: "flex", alignItems: "center", gap: 4, padding: "2px 7px", borderRadius: 5, fontSize: 9, fontWeight: 600, backgroundColor: G_BG, color: GD, border: "none", cursor: "pointer", fontFamily: "inherit" }}>PLZ: {filterPlz.join(", ")}<X size={8} strokeWidth={2.5} /></button>}
                      {filterRedMonat && <button onClick={() => setFilterRedMonat(null)} style={{ display: "flex", alignItems: "center", gap: 4, padding: "2px 7px", borderRadius: 5, fontSize: 9, fontWeight: 600, backgroundColor: G_BG, color: GD, border: "none", cursor: "pointer", fontFamily: "inherit" }}>{filterRedMonat}<X size={8} strokeWidth={2.5} /></button>}
                    </div>
                  )}
                  ...
                  <div style={{ maxHeight: 220, overflowY: "auto", scrollbarWidth: "none" as const }}>
                    {filteredMarkets.length === 0 ? (
                      ...
                      filteredMarkets.map((m) => {
                        ...
                        return (
                          <div key={m.id} onClick={() => toggleMarket(m.id)} style={{ display: "flex", alignItems: "center", gap: 10, padding: "7px 4px", cursor: "pointer", borderBottom: "1px solid rgba(0,0,0,0.03)", borderRadius: 4 }} onMouseEnter={(e) => (e.currentTarget.style.backgroundColor = "rgba(0,0,0,0.015)")} onMouseLeave={(e) => (e.currentTarget.style.backgroundColor = "transparent")}>
                            <div style={{ width: 14, height: 14, borderRadius: 4, flexShrink: 0, border: checked ? "none" : "1.5px solid rgba(0,0,0,0.15)", backgroundColor: checked ? GD : "transparent", display: "flex", alignItems: "center", justifyContent: "center", transition: "all 0.15s ease" }}>
                              {checked && <Check size={8} strokeWidth={3} color="#fff" />}
                            </div>
                  ...
                  {scheduleType === "scheduled" && (
                    <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
                      <div>
                        ...
                        <BillaDatePicker value={startDate} onChange={setStartDate} placeholder="Startdatum wählen" />
                      </div>
                      <div>
                        <label style={{ ...labelStyle, display: "flex", alignItems: "center", gap: 5 }}><Calendar size={9} strokeWidth={2} />Bis</label>
                        <BillaDatePicker value={endDate} onChange={setEndDate} placeholder="Enddatum wählen" />
                      </div>
                      {startDate && endDate && (
                        <div style={{ padding: "8px 12px", borderRadius: 8, backgroundColor: G_BG_FAINT, fontSize: 10, color: "rgba(0,0,0,0.4)", lineHeight: 1.5, textAlign: "center" }}>
      ...
      {contextMenu && (
        <BillaLibraryContextMenu x={contextMenu.x} y={contextMenu.y}
          onDetails={() => { const mod = availableModules.find((m) => m.id === contextMenu.moduleId); if (mod) setPreviewModule(mod); }}
          onClose={() => setContextMenu(null)}
        />
      )}

      {previewModule && <BillaModulePreviewModal module={previewModule} onClose={() => setPreviewModule(null)} />}
    </>
  );
}
