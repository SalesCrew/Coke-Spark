...
const T   = "#0891B2";
const TD  = "#0e7490";
const T_BG = "rgba(8,145,178,0.07)";
...
type BillaModule = Module;
type BillaFragebogen = Fragebogen;
...
function BillaModuleDeleteDialog({ module, usedInCount, usedInNames, onDeleteModule, onClose }: {
  module: BillaModule; usedInCount: number; usedInNames: string[];
...
function BillaFragebogenDeleteDialog({ fragebogen, onDeleteOnly, onClose }: {
  fragebogen: BillaFragebogen; onDeleteOnly: () => void; onClose: () => void;
}) {
  const [pending, setPending] = useState<{ label: string; action: () => void } | null>(null);
  useEffect(() => {
    function key(e: KeyboardEvent) { if (e.key === "Escape") { if (pending) setPending(null); else onClose(); } }
    ...
    >
      ...
      <div onClick={() => setExpanded(!expanded)} style={{ display: "flex", alignItems: "center", gap: 12, padding: "14px 18px", cursor: "pointer", userSelect: "none" }}>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 6, flexShrink: 0 }}>
          ...
          <span style={{ fontSize: 9, fontWeight: 600, padding: "3px 8px", borderRadius: 5, backgroundColor: usedInFragebogen.count > 0 ? T_BG : "rgba(0,0,0,0.03)", color: usedInFragebogen.count > 0 ? T : "rgba(0,0,0,0.3)" }}>
              ...
              {module.questions.map((q, qi) => {
                ...
                return (
                  <div key={q.id} style={{ display: "flex", alignItems: "center", gap: 8, padding: "6px 0" }}>
                    <div style={{ width: 18, height: 18, borderRadius: "50%", background: `linear-gradient(to bottom, ${T}, ${TD})`, display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}>
                    ...
                    {q.rules.length > 0 && <Zap size={10} strokeWidth={2} color={T} style={{ flexShrink: 0 }} />}
...
function BillaFragenListItem({ question, moduleName, onDelete }: {
  ...
  return (
    ...
    >
      ...
      >
        ...
        {question.rules.length > 0 && <Zap size={10} strokeWidth={2} color={T} style={{ flexShrink: 0 }} />}
        ...
        {moduleName === "Nicht zugewiesen" ? (
          ...
          <span style={{ fontSize: 9, fontWeight: 500, padding: "2px 8px", borderRadius: 4, backgroundColor: T_BG, color: T, flexShrink: 0, maxWidth: 120, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>
            {moduleName}
          </span>
        )}
...
function BillaFragebogenCard({ fragebogen, moduleList, onEdit, onDuplicate, onDuplicateToStd, onDuplicateToFlex, onDelete }: {
  ...
  return (
    ...
    >
      ...
      {deleteDialog && (
        <BillaFragebogenDeleteDialog fragebogen={fragebogen} onDeleteOnly={onDelete} onClose={() => setDeleteDialog(false)} />
      )}
...
function BillaEmptyState({ tab }: { tab: Tab }) {
  const cfg = {
...
export default function BillaPage() {
  ...
  [...modules, ...stdModules, ...flexModules]
  ...
  const filteredModules = (q
    ...
    : modules).filter((m) => m.id !== "__billa_unassigned__");
  ...
  useEffect(() => {
    ...
    window.addEventListener("admin:billa:export", handler);
    return () => window.removeEventListener("admin:billa:export", handler);
  });
              ...
              return (
                <button key={tab.key} onClick={() => setActiveTab(tab.key)}
                  style={{
                    ...
                    color: isActive ? T : "rgba(0,0,0,0.4)",
                    backgroundColor: "transparent", border: "none",
                    borderBottom: "2px solid transparent",
                    backgroundImage: isActive ? `linear-gradient(#f5f5f7, #f5f5f7), linear-gradient(to right, ${T}, ${TD})` : "none",
                ...
                >
                  {tab.label}
                  {count > 0 && (
                    <span style={{ fontSize: 9, fontWeight: 700, minWidth: 16, height: 16, borderRadius: 8, display: "flex", alignItems: "center", justifyContent: "center", padding: "0 4px", backgroundColor: isActive ? T_BG : "rgba(0,0,0,0.05)", color: isActive ? T : "rgba(0,0,0,0.35)" }}>
                      {count}
                    </span>
                  )}
                </button>
          ...
          {activeTab === "fragen" && (
            <div ref={typeDropRef} style={{ position: "relative", marginBottom: -4 }}>
              <button onClick={() => setTypeDropOpen((o) => !o)}
                style={{ display: "flex", alignItems: "center", gap: 5, paddingTop: 4, paddingBottom: 4, paddingLeft: 8, paddingRight: 6, background: "transparent", border: "none", cursor: "pointer", fontSize: 11, fontFamily: "inherit", color: filterType ? T : "rgba(0,0,0,0.35)", fontWeight: filterType ? 600 : 400 }}
              >
              ...
              {typeDropOpen && (
                <div style={{ position: "absolute", top: "calc(100% + 6px)", right: 0, zIndex: 300, background: "#fff", borderRadius: 10, boxShadow: "0 4px 20px rgba(0,0,0,0.09), 0 1px 4px rgba(0,0,0,0.05)", padding: "6px 0", minWidth: 160 }}>
                  <button onClick={() => { setFilterType(null); setTypeDropOpen(false); }}
                    style={{ display: "flex", alignItems: "center", justifyContent: "space-between", width: "100%", padding: "6px 14px", background: "none", border: "none", cursor: "pointer", fontSize: 11, fontFamily: "inherit", color: !filterType ? T : "rgba(0,0,0,0.55)", fontWeight: !filterType ? 600 : 400 }}
                    onMouseEnter={(e) => (e.currentTarget.style.backgroundColor = "rgba(0,0,0,0.03)")}
                    onMouseLeave={(e) => (e.currentTarget.style.backgroundColor = "transparent")}
                  >
                    Alle Typen
                    {!filterType && <Check size={10} strokeWidth={2.5} color={T} />}
                  </button>
      ...
      <div style={{ marginTop: 16 }}>
          ...
          filteredModules.length === 0 ? (
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
              {filteredModules.map((m) => {
                ...
                return (
                  <BillaModuleCard key={m.id} module={m}
                    onEdit={() => onEdit(m)}
          ...
          filteredQuestions.length === 0 ? (
            ...
            <div style={{ backgroundColor: "#fff", borderRadius: 10, boxShadow: "0 2px 8px rgba(0,0,0,0.04)", overflow: "hidden" }}>
              {filteredQuestions.map(({ question, moduleName }, i) => (
                <BillaFragenListItem key={`${question.id}-${i}`} question={question} moduleName={moduleName}
                  onDelete={() => {
        ...
        {activeTab === "fragebogen" && (
          filteredFragebogen.length === 0 ? (
            <div style={{ backgroundColor: "#fff", borderRadius: 14, boxShadow: "0 2px 8px rgba(0,0,0,0.04)", padding: 20, minHeight: 340, display: "flex", alignItems: "center", justifyContent: "center" }}>
              <BillaEmptyState tab="fragebogen" />
            </div>
          ) : (
