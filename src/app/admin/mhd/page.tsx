...
function MhdDeleteMenu({ x, y, onDelete, onClose }: {
...
function MhdFragenListItem({ question, moduleName, onDelete }: {
  question: Question;
  moduleName: string;
  onDelete?: () => void;
}) {
  ...
  return (
    ...
    >
      ...
      >
        ...
        {question.rules.length > 0 && <Zap size={10} strokeWidth={2} color={PD} style={{ flexShrink: 0 }} />}
        ...
        {moduleName === "Unzugewiesen" ? (
          ...
          <span style={{ fontSize: 9, fontWeight: 500, padding: "2px 8px", borderRadius: 4, backgroundColor: "rgba(124,58,237,0.06)", color: PD, flexShrink: 0, maxWidth: 120, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>
            {moduleName}
          </span>
        )}
    ...
    >
      ...
      <div style={{ display: "flex", alignItems: "center", gap: 12, padding: "14px 18px", cursor: "pointer", userSelect: "none" }} onClick={() => setExpanded(!expanded)}>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 6, flexShrink: 0 }}>
          ...
          <span style={{ fontSize: 9, fontWeight: 600, padding: "3px 8px", borderRadius: 5, backgroundColor: usedInCount > 0 ? P_BG : "rgba(0,0,0,0.03)", color: usedInCount > 0 ? PD : "rgba(0,0,0,0.3)" }}>
                ...
                return (
                  <div key={q.id}>
                    <div style={{ display: "flex", alignItems: "center", gap: 8, padding: "6px 0" }}>
                      ...
                      {q.rules.length > 0 && <Zap size={10} strokeWidth={2} color={PD} style={{ flexShrink: 0 }} />}
...
function MhdFragebogenDeleteDialog({
  fragebogen,
  onDeleteOnly,
  onClose,
}: {
        ...
        <div style={{ padding: "16px 20px", opacity: status === "inactive" ? 0.55 : 1 }}>
          <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 10 }}>
            ...
            <button
              ...
              onMouseEnter={(e) => (e.currentTarget.style.color = PD)}
...
function EmptyState({ tab }: { tab: Tab }) {
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", padding: "60px 20px", gap: 10 }}>
      <div style={{ width: 40, height: 40, borderRadius: "50%", background: P_BG, display: "flex", alignItems: "center", justifyContent: "center" }}>{icon}</div>
            ...
            {TABS.map((tab) => {
              ...
              return (
                <button
                  ...
                  style={{ display: "flex", alignItems: "center", gap: 6, padding: "8px 0", fontSize: 12, fontWeight: isActive ? 650 : 500, color: isActive ? PD : "rgba(0,0,0,0.4)", backgroundColor: "transparent", border: "none", borderBottom: "2px solid transparent", backgroundImage: isActive ? `linear-gradient(#f5f5f7, #f5f5f7), linear-gradient(to right, ${P}, ${PD})` : "none", backgroundOrigin: "padding-box, border-box", backgroundClip: "padding-box, border-box", cursor: "pointer", transition: "all 0.2s ease", letterSpacing: "-0.01em", position: "relative" }}
                >
                  {tab.label}
                  {count > 0 && (
                    <span style={{ fontSize: 9, fontWeight: 700, minWidth: 16, height: 16, borderRadius: 8, display: "flex", alignItems: "center", justifyContent: "center", padding: "0 4px", backgroundColor: isActive ? P_BG : "rgba(0,0,0,0.05)", color: isActive ? PD : "rgba(0,0,0,0.35)" }}>
                      {count}
                    </span>
                  )}
                </button>
          ...
          {activeTab === "fragen" && (
            <div ref={typeDropRef} style={{ position: "relative", marginBottom: -4 }}>
              <button
                ...
                style={{ display: "flex", alignItems: "center", gap: 5, paddingTop: 4, paddingBottom: 4, paddingLeft: 8, paddingRight: 6, background: "transparent", border: "none", cursor: "pointer", fontSize: 11, fontFamily: "inherit", color: filterType ? PD : "rgba(0,0,0,0.35)", fontWeight: filterType ? 600 : 400 }}
              >
              ...
              {typeDropOpen && (
                <div style={{ position: "absolute", top: "calc(100% + 6px)", right: 0, zIndex: 300, background: "#fff", borderRadius: 10, boxShadow: "0 4px 20px rgba(0,0,0,0.09), 0 1px 4px rgba(0,0,0,0.05)", padding: "6px 0", minWidth: 160 }}>
                  <button onClick={() => { setFilterType(null); setTypeDropOpen(false); }} style={{ display: "flex", alignItems: "center", justifyContent: "space-between", width: "100%", padding: "6px 14px", background: "none", border: "none", cursor: "pointer", fontSize: 11, fontFamily: "inherit", color: !filterType ? PD : "rgba(0,0,0,0.55)", fontWeight: !filterType ? 600 : 400 }} onMouseEnter={(e) => (e.currentTarget.style.backgroundColor = "rgba(0,0,0,0.03)")} onMouseLeave={(e) => (e.currentTarget.style.backgroundColor = "transparent")}>
                    Alle Typen
                    {!filterType && <Check size={10} strokeWidth={2.5} color={PD} />}
                  </button>
